...
impl std::fmt::Display for CliCommand {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            CliCommand::Cd => "CD",
            CliCommand::Pwd => "PWD",
            CliCommand::Mkdir => "MKDIR",
            CliCommand::Rmdir => "RMDIR",
            CliCommand::Rm => "RM",
            CliCommand::Cp => "CP",
            CliCommand::Mv => "MV",
            CliCommand::Find => "FIND",
            CliCommand::Locate => "LOCATE",
            CliCommand::Cat => "CAT",
            CliCommand::Less => "LESS",
            CliCommand::More => "MORE",
            CliCommand::Head => "HEAD",
            CliCommand::Tail => "TAIL",
            CliCommand::Grep => "GREP",
            CliCommand::Sed => "SED",
            CliCommand::Awk => "AWK",
            CliCommand::Sort => "SORT",
            CliCommand::Uniq => "UNIQ",
            CliCommand::Ps => "PS",
            CliCommand::Top => "TOP",
            CliCommand::Kill => "KILL",
            CliCommand::Jobs => "JOBS",
            CliCommand::Nohup => "NOHUP",
            CliCommand::Crontab => "CRONTAB",
            CliCommand::Systemctl => "SYSTEMCTL",
            CliCommand::Service => "SERVICE",
            CliCommand::Ping => "PING",
            CliCommand::Curl => "CURL",
            CliCommand::Wget => "WGET",
            CliCommand::Ssh => "SSH",
            CliCommand::Scp => "SCP",
            CliCommand::Rsync => "RSYNC",
            CliCommand::Netstat => "NETSTAT",
            CliCommand::Git => "GIT",
            CliCommand::Make => "MAKE",
            CliCommand::Gcc => "GCC",
            CliCommand::Python => "PYTHON",
            CliCommand::Node => "NODE",
            CliCommand::Npm => "NPM",
            CliCommand::Docker => "DOCKER",
            CliCommand::Tar => "TAR",
            CliCommand::Zip => "ZIP",
            CliCommand::Unzip => "UNZIP",
            CliCommand::Gzip => "GZIP",
            CliCommand::Gunzip => "GUNZIP",
            CliCommand::Uname => "UNAME",
            CliCommand::Whoami => "WHOAMI",
            CliCommand::Id => "ID",
            CliCommand::Date => "DATE",
            CliCommand::Uptime => "UPTIME",
            CliCommand::Df => "DF",
            CliCommand::Du => "DU",
            CliCommand::Free => "FREE",
            CliCommand::Vim => "VIM",
            CliCommand::Nano => "NANO",
            CliCommand::Emacs => "EMACS",
        };
        write!(f, "{}", s)
    }
}
...
impl CliUsageAnalyzer {
    ...
    pub fn train_on_efficient_patterns(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "📊 Trained on {} efficient CLI patterns",
    ...
    pub fn analyze_cli_usage(
        ...
        // Store user pattern
        self.user_patterns
            .insert(user_id.to_string(), command_sequence.clone());
        ...
        let optimization_insights =
            self.generate_optimization_insights(&command_sequence, max_anomaly);
    ...
    fn create_efficient_usage_analysis(
        &mut self,
        user_id: &str,
        ...
        let analysis = CliUsageAnalysis {
            ...
            explanation: "Efficient CLI usage pattern demonstrating expert proficiency."
                .to_string(),
    ...
    fn generate_optimization_insights(
        ...
        let cd_count = command_sequence
            .iter()
            .filter(|c| c.as_str() == "CD")
            .count();
        let pwd_count = command_sequence
            .iter()
        ...
        if pwd_count > cd_count {
            insights.push(
                "Consider using shell prompt customization to show current directory".to_string(),
            );
        }

        // Analyze file operations
        let ls_count = command_sequence
            .iter()
            .filter(|c| c.as_str() == "LS")
            .count();
        if ls_count > command_sequence.len() / 3 {
            insights
                .push("Consider using file manager or IDE for frequent file browsing".to_string());
        }

        // Analyze text processing
        let cat_count = command_sequence
            .iter()
            .filter(|c| c.as_str() == "CAT")
            .count();
        let less_count = command_sequence
            .iter()
            .filter(|c| c.as_str() == "LESS")
            .count();
        if cat_count > less_count * 2 {
            insights
                .push("Consider using 'less' instead of 'cat' for viewing large files".to_string());
        }

        // Analyze development workflow
        let git_count = command_sequence
            .iter()
            .filter(|c| c.as_str() == "GIT")
            .count();
        let make_count = command_sequence
            .iter()
            .filter(|c| c.as_str() == "MAKE")
            .count();
        if git_count > 0 && make_count == 0 {
            insights
                .push("Consider using build automation tools for development workflow".to_string());
        }
    ...
    fn detect_power_user_patterns(&self, commands: &[String]) -> bool {
        let advanced_commands = commands
            .iter()
            .filter(|c| {
                c.as_str() == "AWK"
                    || c.as_str() == "SED"
    ...
    pub fn generate_analysis_summary(&self) -> CliAnalysisSummary {
        ...
        let inefficient_users = self
            .analysis_results
            .iter()
            .filter(|r| {
                matches!(
                    r.user_proficiency,
                    UserProficiency::Beginner | UserProficiency::Intermediate
                )
            })
            .count();
    ...
    fn generate_efficient_cli_patterns(&self) -> Vec<Vec<String>> {
            ...
            for base_idx in 0..base_patterns.len() {
                ...
                if !efficiency_commands.is_empty() {
                    let startup_efficiency =
                        &efficiency_commands[base_idx % efficiency_commands.len()];
                ...
                if !efficiency_commands.is_empty() {
                    let final_efficiency =
                        &efficiency_commands[iteration % efficiency_commands.len()];
        ...
        for _day in 0..30 {
            ...
            let string_pattern: Vec<String> =
                full_day_usage.iter().map(|c| c.to_string()).collect();
            patterns.push(string_pattern);
            ...
            let alt_string_pattern: Vec<String> =
                alt_day_usage.iter().map(|c| c.to_string()).collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(analysis) = result {
        println!(
            "✅ User Proficiency: {}",
            analysis.user_proficiency.to_string()
        );
    ...
    println!(
        "Inefficient Users Found: {}",
        summary.inefficient_users_found
    );
    println!(
