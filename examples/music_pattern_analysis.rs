...
impl std::fmt::Display for MusicalElement {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            MusicalElement::C => "C",
            MusicalElement::CSharp => "C_SHARP",
            MusicalElement::D => "D",
            MusicalElement::DSharp => "D_SHARP",
            MusicalElement::E => "E",
            MusicalElement::F => "F",
            MusicalElement::FSharp => "F_SHARP",
            MusicalElement::G => "G",
            MusicalElement::GSharp => "G_SHARP",
            MusicalElement::A => "A",
            MusicalElement::ASharp => "A_SHARP",
            MusicalElement::B => "B",
            MusicalElement::CMajor => "C_MAJOR",
            MusicalElement::CMinor => "C_MINOR",
            MusicalElement::DMajor => "D_MAJOR",
            MusicalElement::DMinor => "D_MINOR",
            MusicalElement::EMajor => "E_MAJOR",
            MusicalElement::EMinor => "E_MINOR",
            MusicalElement::FMajor => "F_MAJOR",
            MusicalElement::FMinor => "F_MINOR",
            MusicalElement::GMajor => "G_MAJOR",
            MusicalElement::GMinor => "G_MINOR",
            MusicalElement::AMajor => "A_MAJOR",
            MusicalElement::AMinor => "A_MINOR",
            MusicalElement::BMajor => "B_MAJOR",
            MusicalElement::BMinor => "B_MINOR",
            MusicalElement::Verse => "VERSE",
            MusicalElement::Chorus => "CHORUS",
            MusicalElement::Bridge => "BRIDGE",
            MusicalElement::Intro => "INTRO",
            MusicalElement::Outro => "OUTRO",
            MusicalElement::Solo => "SOLO",
            MusicalElement::Whole => "WHOLE",
            MusicalElement::Half => "HALF",
            MusicalElement::Quarter => "QUARTER",
            MusicalElement::Eighth => "EIGHTH",
            MusicalElement::Sixteenth => "SIXTEENTH",
            MusicalElement::Rest => "REST",
            MusicalElement::LongRest => "LONG_REST",
        };
        write!(f, "{}", s)
    }
}
...
impl MusicPatternAnalyzer {
    ...
    pub fn train_on_common_patterns(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "📊 Trained on {} common musical patterns",
            common_patterns.len()
        );
    ...
    pub fn analyze_composition(
        ...
        let musical_sequence: Vec<String> = elements.iter().map(|e| e.to_string()).collect();

        // Store composition pattern
        self.composition_patterns
            .insert(composition_id.to_string(), musical_sequence.clone());

        let detection_start = Instant::now();
        let anomalies = self
            .detector
            .detect_anomalies(&musical_sequence, threshold)?;
        ...
        if anomalies.is_empty() {
            ...
            let analysis =
                self.create_common_pattern_analysis(composition_id, &musical_sequence)?;
    ...
    fn create_common_pattern_analysis(
        &mut self,
        composition_id: &str,
        musical_sequence: &[String],
    ) -> Result<MusicalAnalysis, Box<dyn std::error::Error>> {
        let musical_patterns = vec!["COMMON_MUSICAL_PATTERN".to_string()];
        let style_insights = self.generate_style_insights(
            musical_sequence,
            &AnomalyScore {
    ...
    fn generate_style_insights(
        ...
        // Analyze chord usage
        let major_chords = musical_sequence
            .iter()
            .filter(|e| e.contains("MAJOR"))
            .count();
        let minor_chords = musical_sequence
            .iter()
            .filter(|e| e.contains("MINOR"))
            .count();
        ...
        if major_chords > minor_chords * 2 {
            ...
            insights
                .push("Minor tonality emphasis - melancholic or dramatic character".to_string());
        }

        // Analyze structural elements
        let verse_count = musical_sequence
            .iter()
            .filter(|e| e.as_str() == "VERSE")
            .count();
        let chorus_count = musical_sequence
            .iter()
            .filter(|e| e.as_str() == "CHORUS")
            .count();
        ...
        let complex_rhythms = musical_sequence
            .iter()
        ...
        let rest_count = musical_sequence
            .iter()
            .filter(|e| e.contains("REST"))
            .count();
    ...
    fn detect_unusual_chord_progression(&self, sequence: &[String]) -> bool {
        ...
        let chords: Vec<&String> = sequence
            .iter()
            .filter(|e| e.contains("MAJOR") || e.contains("MINOR"))
            .collect();
    ...
    fn detect_experimental_structure(&self, sequence: &[String]) -> bool {
        let structure_elements = sequence
            .iter()
            .filter(|e| {
                e.as_str() == "VERSE"
                    || e.as_str() == "CHORUS"
    ...
    fn detect_modal_elements(&self, sequence: &[String]) -> bool {
        ...
        let total_notes = sequence
            .iter()
            .filter(|e| e.len() == 1 || e.contains("SHARP"))
            .count();
    ...
    pub fn generate_analysis_summary(&self) -> MusicAnalysisSummary {
        ...
        let experimental_compositions = self
            .analysis_results
            .iter()
            .filter(|r| {
                matches!(
                    r.complexity_level,
                    ComplexityLevel::Experimental | ComplexityLevel::Complex
                )
            })
            .count();
    ...
    fn generate_common_musical_patterns(&self) -> Vec<Vec<String>> {
        ...
        let extended_patterns = vec![
            ...
            vec![
                MusicalElement::DMinor,
                MusicalElement::EMinor,
                MusicalElement::FMajor,
                MusicalElement::GMajor,
                MusicalElement::AMinor,
                MusicalElement::BMinor,
                MusicalElement::CMajor,
            ],
            // Jazz ii-V-I
            vec![
        ...
        let genre_patterns = [
            ...
            vec![
                MusicalElement::EMajor,
                MusicalElement::BMajor,
                MusicalElement::AMinor,
                MusicalElement::EMajor,
                MusicalElement::CMajor,
                MusicalElement::GMajor,
                MusicalElement::DMajor,
                MusicalElement::EMajor,
            ],
            // Pop ballad
            vec![
        ...
        for iteration in 0..40 {
            ...
            for base_idx in 0..base_patterns.len() {
                ...
                let string_pattern: Vec<String> =
                    long_sequence.iter().map(|e| e.to_string()).collect();
            ...
            if iteration % 3 == 0 {
                ...
                let string_pattern: Vec<String> =
                    mega_sequence.iter().map(|e| e.to_string()).collect();
                patterns.push(string_pattern);
            }
        }
        ...
        for _composition in 0..25 {
            ...
            let string_pattern: Vec<String> =
                full_composition.iter().map(|e| e.to_string()).collect();
            patterns.push(string_pattern);
            ...
            let alt_string_pattern: Vec<String> =
                alt_composition.iter().map(|e| e.to_string()).collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(analysis) = result {
        println!(
            "✅ Complexity Level: {}",
            analysis.complexity_level.to_string()
        );
    ...
    if let Some(analysis) = result {
        ...
        println!("   Patterns: {:?}", analysis.musical_patterns);
        println!(
            "   Similar Compositions Found: {}",
            analysis.similar_compositions_found
        );
        println!("   Insights: {:?}", analysis.style_insights);
    }
    ...
    println!(
        ...
        summary.total_compositions_analyzed
    );
    println!(
        "Experimental Compositions Found: {}",
        summary.experimental_compositions_found
    );
    println!(
    ...
    println!(
        "Style Insights Generated: {}",
        summary.style_insights_generated
    );
