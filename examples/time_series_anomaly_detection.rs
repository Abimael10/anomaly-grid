...
//!
//! This example demonstrates sophisticated time series anomaly detection using
//! anomaly-grid for discretized time series data. It shows how to convert
//! continuous time series into categorical sequences and detect temporal anomalies
//! with rigorous mathematical validation and proof of correctness.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔢 Discretized to {} categorical states", discretized_series.len());
    ...
    for (scenario_name, (raw_data, test_sequence), is_anomalous, threshold) in test_scenarios {
        ...
        if detected {
            ...
            if let Some(strongest) = anomalies.iter()
    ...
    let validation_passed = accuracy >= 0.8 && f1_score >= 0.75 && 
    ...
    if validation_passed {
        ...
        if accuracy < 0.8 { println!("   - Accuracy below threshold"); }
        if f1_score < 0.75 { println!("   - F1 score below threshold"); }
    }
...
fn generate_time_series_data(length: usize) -> Result<(Vec<f64>, Vec<String>), Box<dyn std::error::Error>> {
    ...
    for (i, &value) in data.iter().enumerate() {
        ...
        let _volatility = if i >= 5 {
            let recent_std = {
                ...
                let recent_var = recent.iter().map(|x| (x - recent_mean).powi(2)).sum::<f64>() / recent.len() as f64;
                recent_var.sqrt()
            };
...
fn validate_discretization_quality(raw_data: &[f64], discretized: &[String]) -> Result<(), Box<dyn std::error::Error>> {
...
fn validate_temporal_consistency(detector: &AnomalyDetector, training_data: &[String]) -> Result<(), Box<dyn std::error::Error>> {
...
fn analyze_temporal_patterns(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pattern_tests = vec![
        ("Stable Pattern", vec!["MEDIUM_NEAR_MEAN_STABLE_NORMAL_VOLATILITY"; 5]),
        ("Rising Pattern", vec!["LOW_BELOW_MEAN_RISING_NORMAL_VOLATILITY", "MEDIUM_NEAR_MEAN_RISING_NORMAL_VOLATILITY", "HIGH_ABOVE_MEAN_RISING_NORMAL_VOLATILITY"]),
        ("Volatile Pattern", vec!["MEDIUM_NEAR_MEAN_STABLE_HIGH_VOLATILITY"; 3]),
    ];
...
fn perform_sensitivity_analysis(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for threshold in thresholds {
        ...
        println!("{:8.3} | {:9} | {:12.4}", threshold, anomalies.len(), max_strength);
    }
...
fn test_temporal_robustness(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let edge_cases = vec![
        ("Constant Values", vec!["MEDIUM_NEAR_MEAN_STABLE_NORMAL_VOLATILITY"; 20]),
        ("Alternating States", vec!["HIGH_ABOVE_MEAN_RISING_NORMAL_VOLATILITY", "LOW_BELOW_MEAN_FALLING_NORMAL_VOLATILITY"].repeat(10)),
        ("Single Transition", vec!["LOW_BELOW_MEAN_STABLE_NORMAL_VOLATILITY", "HIGH_ABOVE_MEAN_STABLE_NORMAL_VOLATILITY"]),
    ];
...
fn benchmark_temporal_performance(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
...
fn validate_mathematical_properties(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for threshold in thresholds {
        ...
        if count > prev_count {
            return Err(format!("Monotonicity violation: threshold {} detected more anomalies than lower threshold", threshold).into());
        }
