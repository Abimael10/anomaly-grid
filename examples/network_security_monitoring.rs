...
//!
//! This example demonstrates real-time network security monitoring using the anomaly-grid
//! library. It shows how to detect various types of network attacks and security threats
//! by analyzing network traffic patterns and behaviors.
...
fn generate_insider_threat() -> Vec<String> {
    vec![
        ...
        "BULK_DATA_DOWNLOAD",
        "USB_DEVICE_INSERT",
        "PERSONAL_EMAIL_FORWARD",
        "CLOUD_UPLOAD_PERSONAL",
        "POLICY_VIOLATION",
        "AUDIT_LOG_ACCESS",
...
fn classify_threat(max_strength: f64, avg_information: f64, anomaly_count: usize) -> (String, f64) {
    let base_score =
        max_strength * 0.4 + (avg_information / 10.0) * 0.3 + (anomaly_count as f64 / 20.0) * 0.3;
...
fn calculate_false_positive_rate(
