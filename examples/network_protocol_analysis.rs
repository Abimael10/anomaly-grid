...
impl ProtocolState {
    fn to_string(&self) -> String {
        match self {
            ProtocolState::TcpSyn => "TCP_SYN".to_string(),
            ProtocolState::TcpSynAck => "TCP_SYN_ACK".to_string(),
            ProtocolState::TcpAck => "TCP_ACK".to_string(),
            ProtocolState::TcpEstablished => "TCP_ESTABLISHED".to_string(),
            ProtocolState::TcpFin => "TCP_FIN".to_string(),
            ProtocolState::TcpFinAck => "TCP_FIN_ACK".to_string(),
            ProtocolState::TcpRst => "TCP_RST".to_string(),
            ProtocolState::TcpClosed => "TCP_CLOSED".to_string(),
            ProtocolState::HttpRequest => "HTTP_REQUEST".to_string(),
            ProtocolState::HttpResponse => "HTTP_RESPONSE".to_string(),
            ProtocolState::HttpRedirect => "HTTP_REDIRECT".to_string(),
            ProtocolState::HttpError => "HTTP_ERROR".to_string(),
            ProtocolState::HttpKeepAlive => "HTTP_KEEP_ALIVE".to_string(),
            ProtocolState::HttpClose => "HTTP_CLOSE".to_string(),
            ProtocolState::TlsClientHello => "TLS_CLIENT_HELLO".to_string(),
            ProtocolState::TlsServerHello => "TLS_SERVER_HELLO".to_string(),
            ProtocolState::TlsCertificate => "TLS_CERTIFICATE".to_string(),
            ProtocolState::TlsKeyExchange => "TLS_KEY_EXCHANGE".to_string(),
            ProtocolState::TlsFinished => "TLS_FINISHED".to_string(),
            ProtocolState::TlsApplicationData => "TLS_APPLICATION_DATA".to_string(),
            ProtocolState::TlsAlert => "TLS_ALERT".to_string(),
            ProtocolState::DnsQuery => "DNS_QUERY".to_string(),
            ProtocolState::DnsResponse => "DNS_RESPONSE".to_string(),
            ProtocolState::DnsError => "DNS_ERROR".to_string(),
            ProtocolState::DnsTimeout => "DNS_TIMEOUT".to_string(),
            ProtocolState::AppConnect => "APP_CONNECT".to_string(),
            ProtocolState::AppAuthenticate => "APP_AUTHENTICATE".to_string(),
            ProtocolState::AppDataTransfer => "APP_DATA_TRANSFER".to_string(),
            ProtocolState::AppHeartbeat => "APP_HEARTBEAT".to_string(),
            ProtocolState::AppDisconnect => "APP_DISCONNECT".to_string(),
            ProtocolState::AppError => "APP_ERROR".to_string(),
        }
    }
}
...
impl NetworkProtocolAnalyzer {
    ...
    pub fn train_on_compliant_protocols(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("📊 Trained on {} compliant protocol patterns", compliant_patterns.len());
        
        Ok(())
    }
    
    /// Analyze a protocol flow
    pub fn analyze_protocol_flow(&mut self, states: &[ProtocolState], flow_id: &str, threshold: f64) 
        -> Result<Option<ProtocolFlowAnalysis>, Box<dyn std::error::Error>> {
        
        let protocol_sequence: Vec<String> = states.iter()
            .map(|s| s.to_string())
            .collect();
        
        // Store protocol flow
        self.protocol_flows.insert(flow_id.to_string(), protocol_sequence.clone());
        
        let detection_start = Instant::now();
        let anomalies = self.detector.detect_anomalies(&protocol_sequence, threshold)?;
        ...
        let protocol_violations = self.identify_protocol_violations(&protocol_sequence, max_anomaly);
        
        // Generate optimization insights
        let optimization_insights = self.generate_optimization_insights(&protocol_sequence, max_anomaly);
    ...
    fn create_compliant_flow_analysis(&mut self, flow_id: &str, protocol_sequence: &[String]) 
        -> Result<ProtocolFlowAnalysis, Box<dyn std::error::Error>> {
        
        let protocol_violations = vec![];
        let optimization_insights = self.generate_optimization_insights(protocol_sequence, &AnomalyScore {
    ...
    fn identify_protocol_violations(&self, protocol_sequence: &[String], anomaly: &AnomalyScore) -> Vec<String> {
    ...
    /// Generate optimization insights
    fn generate_optimization_insights(&self, protocol_sequence: &[String], anomaly: &AnomalyScore) -> Vec<String> {
        let mut insights = Vec::new();
        
        // Analyze connection efficiency
        let connection_ops = protocol_sequence.iter()
            .filter(|s| s.contains("CONNECT") || s.contains("SYN"))
            .count();
        let data_ops = protocol_sequence.iter()
        ...
        let error_count = protocol_sequence.iter()
        ...
        // Analyze TLS usage
        let tls_count = protocol_sequence.iter().filter(|s| s.contains("TLS")).count();
        let http_count = protocol_sequence.iter().filter(|s| s.contains("HTTP")).count();
        if http_count > 0 && tls_count == 0 {
            insights.push("Consider using HTTPS for secure communication".to_string());
        }
        
        // Analyze keep-alive usage
        let keep_alive_count = protocol_sequence.iter().filter(|s| s.contains("KEEP_ALIVE")).count();
    ...
    fn detect_tcp_handshake_violation(&self, states: &[String]) -> bool {
        ...
        for i in 0..states.len().saturating_sub(2) {
            if states[i] == "TCP_SYN" {
                ...
                if i + 2 < states.len() {
                    if states[i + 1] != "TCP_SYN_ACK" || states[i + 2] != "TCP_ACK" {
                        return true;
                    }
                }
            }
    ...
    fn detect_improper_termination(&self, states: &[String]) -> bool {
        ...
        states.iter().any(|s| s.contains("RST")) && 
        !states.iter().any(|s| s.contains("FIN"))
    }
    ...
    fn detect_state_violations(&self, states: &[String]) -> bool {
        ...
        for i in 0..states.len() {
            if states[i].contains("DATA") || states[i].contains("TRANSFER") {
                ...
                let established = states[0..i].iter()
    ...
    fn count_similar_flows(&self, target_sequence: &[String]) -> usize {
        self.protocol_flows.values()
            .filter(|sequence| {
                let common_states = sequence.iter()
    ...
    pub fn generate_analysis_summary(&self) -> ProtocolAnalysisSummary {
        ...
        let non_compliant_flows = self.analysis_results.iter()
            .filter(|r| matches!(r.compliance_level, ComplianceLevel::NonCompliant | ComplianceLevel::MajorViolations))
            .count();
        ...
        let total_states: usize = self.analysis_results.iter()
    ...
    fn generate_compliant_protocol_patterns(&self) -> Vec<Vec<String>> {
        ...
        let base_patterns = vec![
            ...
            vec![ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed],
            
            // HTTP over TCP
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpClose],
            
            // HTTPS with TLS
            vec![ProtocolState::TcpEstablished, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello, ProtocolState::TlsCertificate, ProtocolState::TlsFinished, ProtocolState::HttpRequest, ProtocolState::HttpResponse],
            
            // DNS query
            vec![ProtocolState::DnsQuery, ProtocolState::DnsResponse],
            
            // Application flow
            vec![ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::AppDisconnect],
            
            // Keep-alive HTTP
            vec![ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive, ProtocolState::HttpRequest, ProtocolState::HttpResponse],
            
            // Error recovery
            vec![ProtocolState::TcpEstablished, ProtocolState::AppError, ProtocolState::AppConnect, ProtocolState::AppDataTransfer],
        ];
        
        // Extended protocol patterns for comprehensive training
        let extended_patterns = vec![
            // Complex HTTPS session
            vec![ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello, ProtocolState::TlsCertificate, ProtocolState::TlsKeyExchange, ProtocolState::TlsFinished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed],
            
            // DNS with fallback
            vec![ProtocolState::DnsQuery, ProtocolState::DnsTimeout, ProtocolState::DnsQuery, ProtocolState::DnsResponse],
            
            // Application with heartbeat
            vec![ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::AppDisconnect],
            
            // HTTP redirect flow
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpRedirect, ProtocolState::TcpClosed, ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse],
            
            // TLS renegotiation
            vec![ProtocolState::TlsApplicationData, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello, ProtocolState::TlsFinished, ProtocolState::TlsApplicationData],
            
            // Multi-request HTTP session
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpClose],
            
            // Application error recovery
            vec![ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppError, ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppDisconnect],
            
            // TCP with retransmission
            vec![ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::TcpAck, ProtocolState::TcpAck, ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed],
            
            // DNS over HTTPS
            vec![ProtocolState::TcpEstablished, ProtocolState::TlsApplicationData, ProtocolState::DnsQuery, ProtocolState::DnsResponse, ProtocolState::TlsApplicationData],
            
            // WebSocket upgrade
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::AppConnect, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::AppDataTransfer, ProtocolState::AppDisconnect],
            
            // Load balancer health check
            vec![ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed],
            
            // API authentication flow
            vec![ProtocolState::TcpEstablished, ProtocolState::TlsApplicationData, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::TlsApplicationData],
        ];
        
        // Real-world scenario patterns
        let scenario_patterns = vec![
            // Web browsing session
            vec![ProtocolState::DnsQuery, ProtocolState::DnsResponse, ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello, ProtocolState::TlsCertificate, ProtocolState::TlsFinished, ProtocolState::HttpRequest, ProtocolState::HttpResponse],
            
            // File download
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::HttpClose],
            
            // Video streaming
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat],
            
            // Email client sync
            vec![ProtocolState::TcpEstablished, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello, ProtocolState::TlsFinished, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::AppDisconnect],
            
            // Database connection
            vec![ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppDataTransfer, ProtocolState::AppDisconnect, ProtocolState::TcpClosed],
            
            // Microservice communication
            vec![ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpClose],
            
            // CDN cache miss
            vec![ProtocolState::HttpRequest, ProtocolState::HttpRedirect, ProtocolState::TcpSyn, ProtocolState::TcpSynAck, ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse],
        ...
        let optimization_states = vec![
            ...
            vec![ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData],
            ...
            for base_idx in 0..base_patterns.len() {
                ...
                for segment in 0..num_segments {
                    ...
                    if !optimization_states.is_empty() {
                        let optimization = &optimization_states[segment % optimization_states.len()];
                ...
                if !optimization_states.is_empty() {
                    let final_optimization = &optimization_states[iteration % optimization_states.len()];
        ...
        for _session in 0..35 {
            ...
            full_day_traffic.extend(vec![
                ProtocolState::DnsQuery, ProtocolState::DnsResponse, ProtocolState::TcpSyn, ProtocolState::TcpSynAck,
                ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello,
                ProtocolState::TlsCertificate, ProtocolState::TlsFinished, ProtocolState::HttpRequest, ProtocolState::HttpResponse,
                ProtocolState::HttpKeepAlive, ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer,
                ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData
            ]);
            
            // Peak usage traffic (20-25 states)
            full_day_traffic.extend(vec![
                ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive,
                ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat,
                ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::TlsApplicationData, ProtocolState::AppDataTransfer,
                ProtocolState::HttpKeepAlive, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::HttpRequest,
                ProtocolState::HttpResponse, ProtocolState::TlsApplicationData, ProtocolState::AppDataTransfer, ProtocolState::HttpKeepAlive,
                ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::HttpRequest, ProtocolState::HttpResponse,
                ProtocolState::TlsApplicationData
            ]);
            
            // Afternoon application traffic (18-22 states)
            full_day_traffic.extend(vec![
                ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat,
                ProtocolState::AppDataTransfer, ProtocolState::TlsApplicationData, ProtocolState::HttpRequest, ProtocolState::HttpResponse,
                ProtocolState::HttpKeepAlive, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData,
                ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat,
                ProtocolState::TlsApplicationData, ProtocolState::HttpKeepAlive, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat,
                ProtocolState::TlsApplicationData, ProtocolState::AppDisconnect
            ]);
            
            // Evening cleanup and background sync (15-18 states)
            full_day_traffic.extend(vec![
                ProtocolState::AppConnect, ProtocolState::AppAuthenticate, ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat,
                ProtocolState::AppDataTransfer, ProtocolState::AppHeartbeat, ProtocolState::AppDataTransfer, ProtocolState::AppDisconnect,
                ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed, ProtocolState::HttpClose,
                ProtocolState::TlsApplicationData, ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed,
                ProtocolState::AppDisconnect, ProtocolState::HttpClose
            ]);
            
            let string_pattern: Vec<String> = full_day_traffic.iter()
                .map(|s| s.to_string())
                .collect();
            patterns.push(string_pattern);
            ...
            alt_day_traffic.extend(vec![
                ProtocolState::DnsQuery, ProtocolState::DnsResponse, ProtocolState::TcpSyn, ProtocolState::TcpSynAck,
                ProtocolState::TcpAck, ProtocolState::TcpEstablished, ProtocolState::TlsClientHello, ProtocolState::TlsServerHello,
                ProtocolState::TlsCertificate, ProtocolState::TlsKeyExchange, ProtocolState::TlsFinished, ProtocolState::HttpRequest,
                ProtocolState::HttpResponse, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData,
                ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData,
                ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::TlsApplicationData, ProtocolState::HttpClose,
                ProtocolState::TcpFin, ProtocolState::TcpFinAck, ProtocolState::TcpClosed, ProtocolState::AppDisconnect,
                ProtocolState::HttpClose, ProtocolState::TcpClosed
            ]);
            
            // Video streaming session (22-28 states)
            alt_day_traffic.extend(vec![
                ProtocolState::TcpEstablished, ProtocolState::HttpRequest, ProtocolState::HttpResponse, ProtocolState::HttpKeepAlive,
                ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat,
                ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat,
                ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat,
                ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::TlsApplicationData, ProtocolState::HttpKeepAlive,
                ProtocolState::TlsApplicationData, ProtocolState::AppHeartbeat, ProtocolState::HttpClose, ProtocolState::TcpFin,
                ProtocolState::TcpFinAck, ProtocolState::TcpClosed, ProtocolState::AppDisconnect, ProtocolState::HttpClose
            ]);
            
            let alt_string_pattern: Vec<String> = alt_day_traffic.iter()
                .map(|s| s.to_string())
                .collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(analysis) = result {
        println!("✅ Compliance Level: {}", analysis.compliance_level.to_string());
    ...
    if let Some(analysis) = result {
        println!("⚠️  Compliance Level: {}", analysis.compliance_level.to_string());
    ...
    println!("Non-Compliant Flows Found: {}", summary.non_compliant_flows_found);
