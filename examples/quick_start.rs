//! Quick Start Example for Anomaly Grid
//!
//! This example demonstrates the basic usage of anomaly-grid with realistic
//! patterns and proper threshold tuning for accurate anomaly detection.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📚 Generated {} training sequences with realistic patterns", normal_patterns.len());
...
fn demonstrate_threshold_sensitivity(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
...
fn validate_mathematical_properties(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Validating mathematical properties for {} anomalies:", anomalies.len());
    ...
    for (i, anomaly) in anomalies.iter().enumerate() {
        ...
        assert!(anomaly.information_score >= 0.0,
               "Information score negative: {}", anomaly.information_score);
        
        // Validate numerical stability
        assert!(anomaly.likelihood.is_finite(), "Likelihood not finite");
        assert!(anomaly.anomaly_strength.is_finite(), "Anomaly strength not finite");
        assert!(anomaly.information_score.is_finite(), "Information score not finite");
        ...
        println!("   ✅ Anomaly {} passes all mathematical validations", i + 1);
    }
