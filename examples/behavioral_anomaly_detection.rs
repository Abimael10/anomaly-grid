...
//!
//! This example demonstrates advanced behavioral anomaly detection using the anomaly-grid
//! library for cybersecurity applications. It simulates real-world scenarios including
...
fn validate_training_data_quality(
...
fn generate_social_engineering() -> Vec<String> {
    vec![
        ...
        "USER_PROFILE_SCRAPING",
        "CONTACT_INFO_HARVEST",
        "PHISHING_EMAIL_CRAFT",
        "SOCIAL_MEDIA_RESEARCH",
        "PRETEXTING_PREPARATION",
...
fn validate_anomaly_mathematical_properties(
    ...
    for (i, anomaly) in anomalies.iter().enumerate() {
        ...
        if anomaly.likelihood > 0.0 {
            ...
            if error > 1e-10 {
                return Err(format!(
                    "Anomaly {} log-likelihood inconsistent: error = {:.2e}",
                    i, error
                )
                .into());
            }
...
fn perform_behavioral_pattern_analysis(
    detector: &AnomalyDetector,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pattern_tests = vec![
        (
            "Sequential Login Pattern",
            vec!["LOGIN", "CHECK_EMAIL", "LOGOUT"],
        ),
        (
            "Work Session Pattern",
            vec!["LOGIN", "PROJECT_ACCESS", "CODE_EDIT", "LOGOUT"],
        ),
        (
            "Admin Pattern",
            vec!["LOGIN", "ADMIN_PANEL", "USER_MANAGE", "LOGOUT"],
        ),
    ];
...
fn optimize_detection_threshold(
    detector: &AnomalyDetector,
...
fn test_detection_robustness(detector: &AnomalyDetector) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let edge_cases = vec![
        ...
        (
            "Unknown Actions",
            vec!["UNKNOWN_ACTION_1", "UNKNOWN_ACTION_2"],
        ),
    ];
