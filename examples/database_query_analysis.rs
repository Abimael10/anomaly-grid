...
impl std::fmt::Display for SqlOperation {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            SqlOperation::SelectJoin => "SELECT_JOIN",
            SqlOperation::SelectAggregate => "SELECT_AGGREGATE",
            SqlOperation::SelectSubquery => "SELECT_SUBQUERY",
            SqlOperation::Insert => "INSERT",
            SqlOperation::InsertBatch => "INSERT_BATCH",
            SqlOperation::Update => "UPDATE",
            SqlOperation::UpdateBatch => "UPDATE_BATCH",
            SqlOperation::Delete => "DELETE",
            SqlOperation::DeleteBatch => "DELETE_BATCH",
            ...
            SqlOperation::CreateIndex => "CREATE_INDEX",
            SqlOperation::DropIndex => "DROP_INDEX",
            SqlOperation::Analyze => "ANALYZE",
            SqlOperation::Vacuum => "VACUUM",
            SqlOperation::Explain => "EXPLAIN",
            SqlOperation::ShowTables => "SHOW_TABLES",
            SqlOperation::Connect => "CONNECT",
            SqlOperation::Disconnect => "DISCONNECT",
            SqlOperation::SetVariable => "SET_VARIABLE",
        };
        write!(f, "{}", s)
    }
}
...
impl DatabaseQueryAnalyzer {
    ...
    pub fn train_on_efficient_patterns(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            "📊 Trained on {} efficient query patterns",
    ...
    pub fn analyze_query_pattern(
        &mut self,
        queries: &[SqlOperation],
        app_id: &str,
        threshold: f64,
    ) -> Result<Option<QueryPatternAnalysis>, Box<dyn std::error::Error>> {
        let query_sequence: Vec<String> = queries.iter().map(|q| q.to_string()).collect();

        // Store query pattern
        self.query_patterns
            .insert(app_id.to_string(), query_sequence.clone());
        ...
        let optimization_patterns =
            self.identify_optimization_patterns(&query_sequence, max_anomaly);
    ...
    fn create_efficient_pattern_analysis(
        &mut self,
        app_id: &str,
        ...
        let analysis = QueryPatternAnalysis {
            ...
            explanation:
                "Efficient database query pattern following best practices. No optimization needed."
                    .to_string(),
    ...
    fn identify_optimization_patterns(
        &self,
        query_sequence: &[String],
    ...
    fn generate_performance_insights(
        ...
        let transaction_ops = query_sequence
            .iter()
            .filter(|q| q.contains("BEGIN") || q.contains("COMMIT") || q.contains("ROLLBACK"))
            .count();
        let modification_ops = query_sequence
            .iter()
        ...
        if modification_ops > 0 && transaction_ops == 0 {
            insights
                .push("Consider using explicit transactions for data modifications".to_string());
        }

        // Analyze join patterns
        let join_count = query_sequence.iter().filter(|q| q.contains("JOIN")).count();
        let select_count = query_sequence
            .iter()
            .filter(|q| q.contains("SELECT"))
            .count();
        ...
        let single_inserts = query_sequence
            .iter()
            .filter(|q| q.as_str() == "INSERT")
            .count();
        if single_inserts > 5 {
            insights
                .push("Consider using batch INSERT operations for better performance".to_string());
        }

        // Analyze schema operations
        let schema_ops = query_sequence
            .iter()
            .filter(|q| q.contains("CREATE") || q.contains("ALTER") || q.contains("DROP"))
            .count();
        if schema_ops > 2 {
            insights.push(
                "Frequent schema changes may impact performance - consider batching".to_string(),
            );
        }
        ...
        if anomaly.anomaly_strength < 0.2 {
            insights
                .push("Efficient query pattern - following database best practices".to_string());
    ...
    fn detect_missing_transactions(&self, queries: &[String]) -> bool {
        let modification_count = queries
            .iter()
            .filter(|q| q.contains("INSERT") || q.contains("UPDATE") || q.contains("DELETE"))
            .count();
        let transaction_count = queries
            .iter()
    ...
    pub fn generate_analysis_summary(&self) -> QueryAnalysisSummary {
        ...
        let inefficient_patterns = self
            .analysis_results
            .iter()
            .filter(|r| {
                matches!(
                    r.efficiency_level,
                    QueryEfficiency::Inefficient | QueryEfficiency::Suboptimal
                )
            })
            .count();
    ...
    fn generate_efficient_query_patterns(&self) -> Vec<Vec<String>> {
        ...
        let base_patterns = [
            ...
            vec![
                SqlOperation::Select,
                SqlOperation::SelectJoin,
                SqlOperation::SelectAggregate,
            ],
            // Schema management
            vec![
        ...
        for iteration in 0..60 {
            ...
            for base_idx in 0..base_patterns.len() {
                ...
                long_sequence.extend(
                    application_patterns[base_idx % application_patterns.len()]
                        .iter()
                        .cloned(),
                );
                ...
                for segment in 0..num_segments {
                    ...
                    if !optimization_commands.is_empty() {
                        let optimization =
                            &optimization_commands[segment % optimization_commands.len()];
            ...
            if iteration % 3 == 0 {
                ...
                let string_pattern: Vec<String> =
                    mega_sequence.iter().map(|q| q.to_string()).collect();
                patterns.push(string_pattern);
            }
        }
        ...
        for _day in 0..40 {
            ...
            let string_pattern: Vec<String> =
                full_day_operations.iter().map(|q| q.to_string()).collect();
            patterns.push(string_pattern);
            ...
            let alt_string_pattern: Vec<String> =
                alt_day_operations.iter().map(|q| q.to_string()).collect();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(analysis) = result {
        println!(
            "✅ Query Efficiency: {}",
            analysis.efficiency_level.to_string()
        );
    ...
    if let Some(analysis) = result {
        ...
        println!(
            ...
            analysis.similar_patterns_found
        );
        println!("   Insights: {:?}", analysis.performance_insights);
    }
    ...
    println!(
        ...
        summary.inefficient_patterns_found
    );
    println!(
    ...
    println!(
        ...
        summary.optimization_patterns_identified
    );
