...
//!
//! This example demonstrates multi-modal anomaly detection using the anomaly-grid
//! library across different data types and detection modes. It shows how to
//! configure and compare different detection approaches.
...
fn train_multi_modal_detectors(
    detectors: &HashMap<String, AnomalyDetector>,
) -> Result<HashMap<String, TrainingResult>, Box<dyn std::error::Error>> {
...
fn validate_training_convergence(
    results: &HashMap<String, TrainingResult>,
    ...
    for (mode, result) in results {
        ...
        if memory_per_context > 10000.0 {
            // 10KB per context seems excessive
            println!(
                "⚠️ Warning: {} detector has high memory per context: {:.1} bytes",
                mode, memory_per_context
            );
        }
        ...
        if data_per_second < 1000.0 {
            println!(
                "⚠️ Warning: {} detector has slow training: {:.0} elements/sec",
                mode, data_per_second
            );
        }
...
fn perform_cross_validation(
    detectors: &HashMap<String, AnomalyDetector>,
) -> Result<HashMap<String, CrossValidationResult>, Box<dyn std::error::Error>> {
    ...
    for (mode, detector) in detectors {
        ...
        for fold in 0..k_folds {
            ...
            let train_data: Vec<_> = full_dataset[0..start_idx]
                .iter()
            ...
            let train_sequences: Vec<String> =
                train_data.iter().flat_map(|d| d.data.clone()).collect();
        ...
        let std_f1 = {
            let variance = fold_f1_scores
                .iter()
                .map(|x| (x - mean_f1).powi(2))
                .sum::<f64>()
                / fold_f1_scores.len() as f64;
            variance.sqrt()
        };
...
fn benchmark_multi_modal_performance(
    detectors: &HashMap<String, AnomalyDetector>,
) -> Result<HashMap<String, PerformanceResult>, Box<dyn std::error::Error>> {
...
fn test_multi_modal_robustness(
    detectors: &HashMap<String, AnomalyDetector>,
) -> Result<HashMap<String, RobustnessResult>, Box<dyn std::error::Error>> {
...
fn perform_final_certification(
    ...
    let mean_cv_accuracy =
        cv_results.values().map(|r| r.mean_accuracy).sum::<f64>() / cv_results.len() as f64;
    let mean_cv_f1 = cv_results.values().map(|r| r.mean_f1).sum::<f64>() / cv_results.len() as f64;

    let mean_throughput = performance_results
        .values()
        .map(|r| r.throughput)
        .sum::<f64>()
        / performance_results.len() as f64;
    let mean_robustness = robustness_results
        .values()
        .map(|r| r.robustness_score)
        .sum::<f64>()
        / robustness_results.len() as f64;
    ...
    let all_passed = accuracy_passed
        && cv_accuracy_passed
        && f1_passed
        && throughput_passed
        && robustness_passed
...
fn generate_validation_report(
    ...
    println!(
        ...
        if certification.accuracy_passed {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        ...
        if certification.cv_accuracy_passed {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "  Cross-Validation F1 Score: {:.3} {}",
        certification.mean_cv_f1,
        if certification.f1_passed {
            "✅"
        } else {
            "❌"
        }
    ...
    println!("\n⚡ PERFORMANCE METRICS:");
    println!(
        "  Mean Throughput: {:.0} detections/sec {}",
        certification.mean_throughput,
        if certification.throughput_passed {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        "  Mean Robustness Score: {:.1}% {}",
        certification.mean_robustness * 100.0,
        if certification.robustness_passed {
            "✅"
        } else {
            "❌"
        }
    ...
    println!(
        ...
        if certification.significance_passed {
            "✅"
        } else {
            "❌"
        }
    ...
    if certification.all_passed {
        ...
        if !certification.accuracy_passed {
            println!("    - Test accuracy below threshold");
        }
        if !certification.cv_accuracy_passed {
            println!("    - Cross-validation accuracy below threshold");
        }
        if !certification.f1_passed {
        ...
        if !certification.robustness_passed {
            println!("    - Robustness below threshold");
        }
        if !certification.significance_passed {
            println!("    - Results not statistically significant");
        }
    }
...
fn generate_training_data_for_mode(mode: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    match mode {
        "sequential" => Ok(vec!["A", "B", "C", "D", "E"]
            .repeat(100)
            .iter()
            .map(|s| s.to_string())
            .collect()),
        "behavioral" => Ok(vec!["LOGIN", "WORK", "EMAIL", "LOGOUT"]
            .repeat(150)
            .iter()
            .map(|s| s.to_string())
            .collect()),
        "temporal" => Ok(vec!["MORNING", "WORK", "LUNCH", "WORK", "EVENING"]
            .repeat(120)
            .iter()
            .map(|s| s.to_string())
            .collect()),
        "network" => Ok(vec!["CONNECT", "AUTH", "DATA", "CLOSE"]
            .repeat(200)
            .iter()
            .map(|s| s.to_string())
            .collect()),
...
fn generate_test_scenarios_for_mode(
    mode: &str,
) -> Result<Vec<(String, Vec<String>, bool, f64)>, Box<dyn std::error::Error>> {
    match mode {
        "sequential" => Ok(vec![
            (
                "Normal Sequence".to_string(),
                vec!["A", "B", "C", "D"]
                    .iter()
                    .map(|s| s.to_string())
                    .collect(),
        ...
        "temporal" => Ok(vec![
            (
                "Normal Time".to_string(),
                vec!["MORNING", "WORK", "EVENING"]
                    .iter()
                    .map(|s| s.to_string())
                    .collect(),
                false,
        ...
        "network" => Ok(vec![
            (
                "Normal Traffic".to_string(),
                vec!["CONNECT", "AUTH", "DATA"]
                    .iter()
                    .map(|s| s.to_string())
                    .collect(),
                false,
            ...
            (
                "Attack Traffic".to_string(),
                vec!["SCAN", "EXPLOIT", "BACKDOOR"]
                    .iter()
                    .map(|s| s.to_string())
                    .collect(),
                true,
...
fn generate_test_data_for_mode(mode: &str) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    match mode {
        ...
        "behavioral" => Ok(vec!["LOGIN", "WORK"]
            .iter()
            .map(|s| s.to_string())
            .collect()),
        "temporal" => Ok(vec!["MORNING", "WORK"]
            .iter()
            .map(|s| s.to_string())
            .collect()),
        "network" => Ok(vec!["CONNECT", "AUTH"]
            .iter()
            .map(|s| s.to_string())
            .collect()),
        _ => Err("Unknown mode".into()),
...
fn generate_cross_validation_dataset_for_mode(
    mode: &str,
) -> Result<Vec<CrossValidationItem>, Box<dyn std::error::Error>> {
    let mut dataset = Vec::new();

    // Generate normal examples
    for i in 0..50 {
        let data = match mode {
            "sequential" => vec!["A", "B", "C", "D"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            "behavioral" => vec!["LOGIN", "WORK", "LOGOUT"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            "temporal" => vec!["MORNING", "WORK", "EVENING"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            "network" => vec!["CONNECT", "AUTH", "DATA"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            _ => return Err("Unknown mode".into()),
        };
        dataset.push(CrossValidationItem {
            data,
            is_anomalous: false,
        });
    }
    ...
    for i in 0..20 {
        let data = match mode {
            "sequential" => vec!["X", "Y", "Z"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            "behavioral" => vec!["HACK", "STEAL", "ESCAPE"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            "temporal" => vec!["MIDNIGHT", "WORK", "MIDNIGHT"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            "network" => vec!["SCAN", "EXPLOIT", "BACKDOOR"]
                .iter()
                .map(|s| format!("{}_{}", s, i % 3))
                .collect(),
            _ => return Err("Unknown mode".into()),
...
fn generate_edge_cases_for_mode(
    _mode: &str,
    ...
    let base_cases = vec![
        ...
        (
            "Repeated Element".to_string(),
            vec!["REPEAT".to_string(); 10],
        ),
    ];
