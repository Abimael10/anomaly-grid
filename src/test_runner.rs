...
fn test_network_traffic_anomalies() {
    ...
    let normal_traffic: Vec<String> = vec![
        "HTTP_GET", "HTTP_RESPONSE", "HTTPS_GET", "HTTPS_RESPONSE",
        "HTTP_GET", "HTTP_RESPONSE", "HTTPS_POST", "HTTPS_RESPONSE",
        "HTTP_GET", "HTTP_RESPONSE", "HTTPS_GET", "HTTPS_RESPONSE",
        "TCP_SYN", "TCP_ACK", "TCP_DATA", "TCP_FIN",
        "HTTP_GET", "HTTP_RESPONSE", "HTTPS_GET", "HTTPS_RESPONSE",
    ].into_iter().map(String::from).collect();
    
    // Add anomalous patterns: port scanning, DDoS attempts
    let mut traffic_with_anomalies = normal_traffic.clone();
    traffic_with_anomalies.extend(vec![
        "TCP_SYN", "TCP_RST", "TCP_SYN", "TCP_RST", "TCP_SYN", "TCP_RST", // Port scan
        "HTTP_GET", "HTTP_GET", "HTTP_GET", "HTTP_GET", "HTTP_GET", // Rapid requests
        "UNKNOWN_PROTOCOL", "MALFORMED_PACKET", "BUFFER_OVERFLOW_ATTEMPT",
        "SQL_INJECTION_PATTERN", "XSS_ATTEMPT", "COMMAND_INJECTION",
    ...
    for (i, anomaly) in sorted_anomalies.iter().take(5).enumerate() {
        ...
        println!("      Info Score: {:.4}", anomaly.information_theoretic_score);
...
fn test_user_behavior_patterns() {
    ...
    let normal_session: Vec<String> = vec![
        "LOGIN", "DASHBOARD", "PROFILE_VIEW", "SETTINGS", "LOGOUT",
        "LOGIN", "DASHBOARD", "SEARCH", "ITEM_VIEW", "ADD_TO_CART", "CHECKOUT", "LOGOUT",
        "LOGIN", "DASHBOARD", "MESSAGES", "COMPOSE", "SEND", "LOGOUT",
        "LOGIN", "DASHBOARD", "REPORTS", "DOWNLOAD", "LOGOUT",
        "LOGIN", "DASHBOARD", "HELP", "FAQ", "CONTACT", "LOGOUT",
    ].into_iter().map(String::from).collect();
    
    // Suspicious behavior patterns
    let suspicious_session: Vec<String> = vec![
        "LOGIN", "ADMIN_PANEL", "USER_LIST", "DELETE_USER", "DELETE_USER", // Privilege escalation
        "BULK_DOWNLOAD", "BULK_DOWNLOAD", "BULK_DOWNLOAD", // Data exfiltration
        "LOGIN_FAILED", "LOGIN_FAILED", "LOGIN_FAILED", "LOGIN_FAILED", // Brute force
        "RAPID_CLICKS", "RAPID_CLICKS", "RAPID_CLICKS", // Bot behavior
        "UNUSUAL_HOUR_ACCESS", "SENSITIVE_DATA_ACCESS", "EXPORT_ALL_DATA",
    ].into_iter().map(String::from).collect();
    ...
    if let Some(spectral) = &model.spectral_decomposition {
        ...
        println!("   Stationary distribution size: {}", spectral.stationary_distribution.len());
    }
...
fn test_system_log_analysis() {
    ...
    let normal_logs: Vec<String> = vec![
        ...
        "SERVICE_START", "AUTH_SUCCESS", "FILE_ACCESS", "DB_QUERY", "NETWORK_CONNECT",
        "CRON_START", "BACKUP_BEGIN", "BACKUP_SUCCESS", "CRON_END",
        "SERVICE_RESTART", "AUTH_SUCCESS", "FILE_ACCESS", "LOG_ROTATE",
        "HEALTH_CHECK", "MONITOR_ALERT", "MONITOR_CLEAR", "SERVICE_HEALTHY",
    ].into_iter().map(String::from).collect();
    ...
    test_logs.extend(vec![
        ...
        "MEMORY_LEAK", "CPU_SPIKE", "DISK_FULL", "NETWORK_INTRUSION",
        "SERVICE_CRASH", "SERVICE_CRASH", "SERVICE_CRASH", // Repeated crashes
        "ROOTKIT_DETECTED", "MALWARE_SCAN_POSITIVE", "FIREWALL_BREACH",
        "CONFIG_TAMPERED", "LOG_DELETION", "AUDIT_TRAIL_MISSING",
    ].into_iter().map(String::from).collect::<Vec<String>>());
    ...
    match model.build_context_tree(&training_logs) {
        Ok(()) => println!("✓ System log model trained on {} events", training_logs.len()),
    ...
    let mut critical_anomalies: Vec<_> = anomalies.into_iter()
...
fn test_financial_transaction_patterns() {
    ...
    let normal_transactions: Vec<String> = vec![
        "AUTH", "SMALL_PURCHASE", "CONFIRM", "SETTLEMENT",
        "AUTH", "GROCERY", "CONFIRM", "SETTLEMENT",
        "AUTH", "GAS_STATION", "CONFIRM", "SETTLEMENT",
        "AUTH", "ATM_WITHDRAWAL", "CONFIRM", "SETTLEMENT",
        "AUTH", "ONLINE_PURCHASE", "CONFIRM", "SETTLEMENT",
        "AUTH", "RESTAURANT", "CONFIRM", "SETTLEMENT",
        "AUTH", "SUBSCRIPTION", "CONFIRM", "SETTLEMENT",
    ].into_iter().map(String::from).collect();
    
    // Build training set (reduced size to avoid infinite loops)
    let mut training_transactions = Vec::new();
    for _ in 0..5 {  // Reduced from 20
    ...
    test_transactions.extend(vec![
        ...
        "AUTH", "ROUND_AMOUNT", "ROUND_AMOUNT", "ROUND_AMOUNT", // Structured amounts
        "VELOCITY_ALERT", "AUTH", "AUTH", "AUTH", "AUTH", // Rapid transactions
        "AUTH", "CASH_ADVANCE", "CASH_ADVANCE", "CASH_ADVANCE", // Repeated cash advances
        "AUTH", "HIGH_RISK_MERCHANT", "LARGE_AMOUNT", "CHARGEBACK_RISK",
    ].into_iter().map(String::from).collect::<Vec<String>>());
    ...
    match model.build_context_tree(&training_transactions) {
        Ok(()) => println!("✓ Financial model trained on {} transactions", training_transactions.len()),
    ...
    let mut fraud_scores: Vec<_> = anomalies.into_iter()
        .map(|a| {
...
fn test_dna_sequence_analysis() {
    ...
    if let Some(spectral) = &model.spectral_decomposition {
        println!("   Genetic network mixing time: {:.4}", spectral.mixing_time);
        println!("   Sequence stability (spectral gap): {:.4}", spectral.spectral_gap);
    }
    ...
    for (i, anomaly) in genetic_anomalies.iter().take(4).enumerate() {
        println!("   {}. Sequence: {:?}", i + 1, anomaly.state_sequence);
        println!("      Mutation probability: {:.2e}", 1.0 - anomaly.likelihood);
...
fn test_performance_benchmarks() {
    ...
    for &size in &sizes {
        for &order in &orders {
            ...
            // Add some anomalies
            sequence.extend(vec!["X", "Y", "Z", "X", "Y", "Z"].iter().map(|s| s.to_string()));
            ...
            match model.build_context_tree(&sequence) {
                Ok(()) => {
                    ...
                    println!("   Memory efficiency: {:.2} contexts/sequence", 
...
fn test_batch_processing() {
    ...
    let sequences = vec![
        // Web server logs
        vec!["GET", "200", "POST", "201", "GET", "404", "GET", "500", "GET", "200"]
            .into_iter().map(String::from).collect(),
        
        // Database operations
        vec!["CONNECT", "SELECT", "INSERT", "UPDATE", "COMMIT", "DISCONNECT", "ROLLBACK"]
            .into_iter().map(String::from).collect(),
        
        // Network protocols
        vec!["SYN", "ACK", "DATA", "DATA", "FIN", "RST", "SYN", "SYN_FLOOD"]
            .into_iter().map(String::from).collect(),
        
        // Application events
        vec!["START", "LOAD", "PROCESS", "SAVE", "EXIT", "CRASH", "RESTART", "HANG"]
            .into_iter().map(String::from).collect(),
        
        // User interactions
        vec!["CLICK", "SCROLL", "TYPE", "SUBMIT", "REFRESH", "BOT_PATTERN", "SPAM"]
            .into_iter().map(String::from).collect(),
    ];
    ...
    println!("   Average time per sequence: {:?}", batch_time / sequences.len() as u32);
    ...
    for (i, (original, anomalies)) in sequences.iter().zip(batch_results.iter()).enumerate() {
        ...
        if !anomalies.is_empty() {
            let avg_likelihood: f64 = anomalies.iter().map(|a| a.likelihood).sum::<f64>() / anomalies.len() as f64;
            println!("   Average anomaly likelihood: {:.2e}", avg_likelihood);
            
            // Show most suspicious pattern
            if let Some(most_suspicious) = anomalies.iter().min_by(|a, b| a.likelihood.partial_cmp(&b.likelihood).unwrap()) {
...
mod comprehensive_tests {
    ...
    fn test_mathematical_properties() {
        ...
        println!("   Probability conservation error: {:.2e}", total_prob_error);
        assert!(total_prob_error < 1e-10, "Probabilities should sum to 1");
        
        // Test entropy bounds
        for (_context, node) in &model.contexts {
            let max_entropy = (node.probabilities.len() as f64).log2();
            assert!(node.entropy <= max_entropy + 1e-10, "Entropy exceeds theoretical maximum");
