...
use crate::context_trie::{NodeId, TrieNode};
...
impl MemoryPool {
    ...
    pub fn memory_usage(&self) -> usize {
        ...
        // SmallVec pool
        total +=
            self.small_vecs.capacity() * std::mem::size_of::<SmallVec<[(StateId, usize); 4]>>();
    ...
    pub fn auto_tune(&mut self) {
        ...
        if context_hit_rate < MIN_HIT_RATE && self.stats.context_node_requests > 10 {
            ...
            self.context_nodes
                .reserve(new_capacity - self.context_nodes.capacity());
            self.free_context_nodes
                .reserve(new_capacity - self.free_context_nodes.capacity());
        }
        ...
        if trie_hit_rate < MIN_HIT_RATE && self.stats.trie_node_requests > 10 {
            ...
            self.trie_nodes
                .reserve(new_capacity - self.trie_nodes.capacity());
            self.free_trie_nodes
                .reserve(new_capacity - self.free_trie_nodes.capacity());
        }
        ...
        if small_vec_hit_rate < MIN_HIT_RATE && self.stats.small_vec_requests > 10 {
            ...
            self.small_vecs
                .reserve(new_capacity - self.small_vecs.capacity());
            self.free_small_vecs
                .reserve(new_capacity - self.free_small_vecs.capacity());
        }
    }
}
...
impl PoolStats {
    ...
    pub fn overall_hit_rate(&self) -> f64 {
        let total_requests =
            self.context_node_requests + self.trie_node_requests + self.small_vec_requests;
...
mod tests {
    ...
    fn test_auto_tuning() {
        ...
        assert!(
            ...
            "Hit rate should be low: {context_hit_rate:.2}"
        );
        ...
        assert!(
            pool.context_nodes.capacity() >= initial_capacity,
            "Capacity should not decrease: {} -> {}",
            initial_capacity,
            pool.context_nodes.capacity()
        );
    }
