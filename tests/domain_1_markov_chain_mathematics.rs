//! Domain 1: Markov Chain Mathematics
//!
//! This module implements comprehensive domain-driven tests for Markov chain
//! fundamentals, ensuring our implementation adheres to mathematical principles
//! and probabilistic foundations.
...
fn test_markov_property_comprehensive() -> DomainTestResult {
    ...
    for context in contexts_to_test {
        ...
        let prob = detector
            .model()
            .get_best_context_probability(&context_strings, "C");
    ...
    let prob_long = detector
        .model()
        .get_best_context_probability(&long_context_strings, "C");
    let prob_short = detector
        .model()
        .get_best_context_probability(&short_context_strings, "C");
    ...
    if markov_property_holds && context_order_respected {
        DomainTestResult::pass("Markov property correctly implemented".to_string()).with_details(
            vec![
        ...
        DomainTestResult::fail("Markov property violation detected".to_string()).with_details(vec![
...
fn test_transition_probability_normalization_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "B", "A", "C", "A", "B", "A", "C"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let contexts_to_test = vec![vec!["A"], vec!["B"], vec!["C"]];
    ...
    for context in contexts_to_test {
        ...
        for next_state in &possible_next_states {
            let prob = detector
                .model()
                .get_best_context_probability(&context_strings, next_state);
...
fn test_chapman_kolmogorov_equation() -> DomainTestResult {
    ...
    // Train with a simple pattern
    let training_sequence = vec!["A", "B", "A", "B"]
        .repeat(100)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let prob_b_given_a = detector
        .model()
        .get_best_context_probability(&vec!["A".to_string()], "B");
    let prob_a_given_b = detector
        .model()
        .get_best_context_probability(&vec!["B".to_string()], "A");
    ...
    if chapman_kolmogorov_holds {
        DomainTestResult::pass("Chapman-Kolmogorov equation satisfied".to_string()).with_details(
            vec![
        ...
        DomainTestResult::fail("Chapman-Kolmogorov equation violation".to_string()).with_details(
            vec![
                format!("Expected: {:.6}", expected_likelihood),
                format!("Calculated: {:.6}", sequence_likelihood),
                format!("Error: {:.6}", likelihood_error),
...
fn test_stationarity_and_time_homogeneity() -> DomainTestResult {
    ...
    let prob_c_given_b = detector
        .model()
        .get_best_context_probability(&vec!["B".to_string()], "C");
    let prob_a_given_c = detector
        .model()
        .get_best_context_probability(&vec!["C".to_string()], "A");
    ...
    let stationarity_holds = (prob_b_given_a - expected_prob).abs() < tolerance
    ...
    if stationarity_holds && time_homogeneity_holds {
        DomainTestResult::pass("Stationarity and time homogeneity satisfied".to_string())
            .with_details(vec![
                format!(
                    "P(B|A) = {:.6} (expected ~{:.1})",
                    prob_b_given_a, expected_prob
                ),
                format!(
                    "P(C|B) = {:.6} (expected ~{:.1})",
                    prob_c_given_b, expected_prob
                ),
                format!(
                    "P(A|C) = {:.6} (expected ~{:.1})",
                    prob_a_given_c, expected_prob
                ),
            ])
    } else {
