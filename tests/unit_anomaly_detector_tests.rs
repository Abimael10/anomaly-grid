...
//!
//! These tests focus on the core functionality of the anomaly detector,
//! including training, detection, and configuration validation.
...
fn test_basic_training_and_detection() {
    ...
    let training_sequence = vec!["A", "B", "C", "A", "B", "C"]
        .repeat(20)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let normal_anomalies = detector
        .detect_anomalies(&normal_sequence, 0.0)
    ...
    let anomalous_anomalies = detector
        .detect_anomalies(&anomalous_sequence, 0.0)
        .expect("Failed to detect anomalies");
...
fn test_threshold_behavior() {
    ...
    let training_sequence = vec!["NORMAL", "PATTERN"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let low_threshold_anomalies = detector
        .detect_anomalies(&test_sequence, 0.0)
        .expect("Failed to detect with low threshold");
    let high_threshold_anomalies = detector
        .detect_anomalies(&test_sequence, 0.9)
        .expect("Failed to detect with high threshold");

    assert!(
        high_threshold_anomalies.len() <= low_threshold_anomalies.len(),
...
fn test_invalid_thresholds() {
    ...
    assert!(detector
        .detect_anomalies(&test_sequence, f64::INFINITY)
        .is_err());
}
...
fn test_performance_monitoring() {
    ...
    let _result = detector
        .detect_anomalies_with_monitoring(&test_sequence, 0.1)
