//! Performance tests for training scalability.
//! These tests ensure the library scales well with increasing data sizes.
...
fn test_training_time_complexity_with_data_size() {
    ...
    for i in 1..training_times.len() {
        ...
        let time_ratio =
            training_times[i].as_nanos() as f64 / training_times[i - 1].as_nanos() as f64;
...
fn test_training_memory_efficiency() {
    ...
    for &alphabet_size in &alphabet_sizes {
        for &max_order in &max_orders {
            ...
            println!(
                "Alphabet: {}, Order: {}, Contexts: {}, Memory: {} KB",
                alphabet_size,
                max_order,
                context_count,
                memory_usage / 1024
...
fn test_training_convergence_speed() {
    ...
    for &size in &convergence_sizes {
        ...
        println!(
            "Training size: {}, Contexts learned: {}, Time: {:?}",
        ...
        if size >= 100 {
            assert!(
                context_count > 0,
                "Should learn some contexts with {} training examples",
                size
            );
        }
...
fn test_training_with_different_alphabet_sizes() {
    ...
    for &alphabet_size in &alphabet_sizes {
        ...
        println!(
            "Alphabet size: {}, Training time: {:?}, Contexts: {}, Memory: {} KB",
            alphabet_size,
            training_time,
        ...
        if alphabet_size <= 20 {
            assert!(
                ...
                "Should learn contexts for alphabet size {}",
                alphabet_size
            );
        }
    }
...
fn test_training_with_different_max_orders() {
    ...
    for &max_order in &max_orders {
        ...
        println!(
            "Max order: {}, Training time: {:?}, Contexts: {}, Memory: {} KB",
            max_order,
            training_time,
            metrics.context_count,
            metrics.estimated_memory_bytes / 1024
...
fn test_training_throughput_measurement() {
    ...
    for &size in &training_sizes {
        ...
        println!(
            "Training size: {}, Throughput: {:.0} elements/second",
            size, throughput
        );
...
fn test_training_memory_growth_patterns() {
    ...
    for &size in &data_sizes {
        ...
        assert!(
            memory_usage < 50 * 1024 * 1024, // 50MB limit
    ...
    for i in 1..memory_usages.len() {
        ...
        assert!(
            memory_ratio <= size_ratio * size_ratio * size_ratio * 2.0,
