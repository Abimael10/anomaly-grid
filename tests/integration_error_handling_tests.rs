...
fn test_invalid_threshold_errors() {
    ...
    for &threshold in &invalid_thresholds {
        ...
        assert!(result.is_err(), "Should fail with invalid threshold: {}", threshold);
        ...
        match result.unwrap_err() {
            ...
            _ => panic!("Expected InvalidThreshold error for threshold: {}", threshold),
        }
    }
}
...
fn test_invalid_configuration_errors() {
    ...
    let invalid_config_result = AnomalyGridConfig::default()
        .with_smoothing_alpha(-1.0);
    assert!(invalid_config_result.is_err(), "Should fail with negative smoothing alpha");
    
    // Test invalid max_order
    let invalid_order_result = AnomalyGridConfig::default()
        .with_max_order(0);
    assert!(invalid_order_result.is_err(), "Should fail with max_order = 0");
    
    // Test invalid weights
    let invalid_weights_result = AnomalyGridConfig::default()
        .with_weights(-0.5, 0.5);
    assert!(invalid_weights_result.is_err(), "Should fail with negative weight");
}
...
fn test_batch_processing_error_handling() {
    ...
    assert_eq!(results.len(), sequences.len(), "Should return results for all sequences");
...
fn test_invalid_batch_processing_parameters() {
    ...
    let invalid_config = AnomalyGridConfig::default()
        .with_smoothing_alpha(-1.0);
...
fn test_memory_limit_error_handling() {
    ...
    let config = AnomalyGridConfig::default()
        .with_memory_limit(Some(1024)); // Very small limit
    
    if config.is_ok() {
        let mut detector = AnomalyDetector::with_config(config.unwrap())
            .expect("Failed to create detector");
        
        // Try to train with data that might exceed memory limit
        let large_sequence: Vec<String> = (0..10000)
...
fn test_error_message_quality() {
    ...
    assert!(error_msg.contains("max_order"), "Error message should mention max_order");
    assert!(error_msg.contains("0"), "Error message should mention the invalid value");
    ...
    assert!(result.is_err());
    let error_msg = format!("{}", result.unwrap_err());
    assert!(error_msg.contains("threshold"), "Error message should mention threshold");
    assert!(error_msg.contains("1.5"), "Error message should mention the invalid value");
}
...
fn test_graceful_degradation() {
    ...
    detector.train(&minimal_sequence).expect("Failed to train with minimal data");
    ...
    for (i, test_case) in edge_cases.iter().enumerate() {
        ...
        assert!(result.is_ok(), "Edge case {} should not crash: {:?}", i, test_case);
        
        let anomalies = result.unwrap();
        for anomaly in &anomalies {
            assert!(anomaly.likelihood >= 0.0 && anomaly.likelihood <= 1.0,
                   "Invalid likelihood in edge case {}: {}", i, anomaly.likelihood);
            assert!(anomaly.anomaly_strength >= 0.0 && anomaly.anomaly_strength <= 1.0,
                   "Invalid anomaly strength in edge case {}: {}", i, anomaly.anomaly_strength);
            assert!(anomaly.information_score >= 0.0,
                   "Invalid information score in edge case {}: {}", i, anomaly.information_score);
        }
    }
}