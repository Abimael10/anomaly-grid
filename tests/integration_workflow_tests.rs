...
//!
//! These tests verify that the entire anomaly detection pipeline works correctly
//! for realistic use cases and scenarios.
...
fn test_network_security_workflow() {
    ...
    let normal_traffic = vec![
        ...
        "HTTPS_CONNECT", "TLS_HANDSHAKE", "HTTP_POST", "HTTP_201",
    ].repeat(50).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    let anomalies = detector.detect_anomalies(&attack_traffic, 0.1)
...
fn test_financial_fraud_workflow() {
    ...
    let normal_transactions = vec![
        "AUTH", "PURCHASE", "CONFIRM", "SETTLE",
        "AUTH", "ATM_WITHDRAWAL", "CONFIRM",
        "AUTH", "TRANSFER", "CONFIRM", "SETTLE",
    ].repeat(30).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    let anomalies = detector.detect_anomalies(&fraud_transactions, 0.05)
        .expect("Failed to detect anomalies");
...
fn test_industrial_iot_workflow() {
    ...
    let normal_readings = vec![
        "TEMP_NORMAL", "PRESSURE_NORMAL", "VIBRATION_LOW",
        "TEMP_NORMAL", "PRESSURE_HIGH", "VIBRATION_LOW",
        "TEMP_HIGH", "PRESSURE_NORMAL", "VIBRATION_NORMAL",
    ].repeat(40).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    
    detector.train(&normal_readings).expect("Failed to train detector");
    ...
    let anomalies = detector.detect_anomalies(&failure_pattern, 0.01)
        .expect("Failed to detect anomalies");
    ...
    assert!(!anomalies.is_empty(), "Equipment failure should be detected");
}
...
fn test_system_log_workflow() {
    ...
    let normal_logs = vec![
        "USER_LOGIN", "FILE_ACCESS", "PROCESS_START", "USER_LOGOUT",
        "SYSTEM_BOOT", "SERVICE_START", "NETWORK_CONNECT", "SERVICE_STOP",
        "BACKUP_START", "BACKUP_SUCCESS", "CLEANUP_TEMP",
    ].repeat(25).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    let anomalies = detector.detect_anomalies(&incident_logs, 0.01)
        .expect("Failed to detect anomalies");
    ...
    assert!(!anomalies.is_empty(), "Security incidents should be detected");
}
...
fn test_edge_case_workflows() {
    ...
    detector.train(&minimal_training).expect("Failed to train with minimal data");
    
    // Test edge cases
    let empty_test: Vec<String> = vec![];
    let empty_anomalies = detector.detect_anomalies(&empty_test, 0.1)
        .expect("Failed to detect anomalies");
    assert!(empty_anomalies.is_empty(), "Empty sequence should have no anomalies");
    
    let short_test = vec!["X".to_string()];
    let short_anomalies = detector.detect_anomalies(&short_test, 0.1)
        .expect("Failed to detect anomalies");
    ...
    let known_anomalies = detector.detect_anomalies(&known_test, 0.1)
        .expect("Failed to detect anomalies");
...
fn test_sequence_boundary_preservation() {
    ...
    let sequences = vec![
        vec!["SEQ1_START".to_string(), "SEQ1_MID".to_string(), "SEQ1_END".to_string()],
        vec!["SEQ2_START".to_string(), "SEQ2_MID".to_string(), "SEQ2_END".to_string()],
        vec!["SEQ3_START".to_string(), "SEQ3_MID".to_string(), "SEQ3_END".to_string()],
    ];
    
    detector.train_sequences(&sequences).expect("Failed to train sequences");
    ...
    for (i, sequence) in sequences.iter().enumerate() {
        ...
        for anomaly in &anomalies {
            assert!(anomaly.likelihood >= 0.0 && anomaly.likelihood <= 1.0,
                   "Invalid likelihood in sequence {}: {}", i, anomaly.likelihood);
            assert!(anomaly.anomaly_strength >= 0.0 && anomaly.anomaly_strength <= 1.0,
                   "Invalid anomaly strength in sequence {}: {}", i, anomaly.anomaly_strength);
            assert!(anomaly.information_score >= 0.0,
                   "Invalid information score in sequence {}: {}", i, anomaly.information_score);
        }
    }
}
...
fn test_batch_processing_workflow() {
    let sequences = vec![
        vec!["NORMAL", "PATTERN", "A"].iter().map(|s| s.to_string()).collect(),
        vec!["NORMAL", "PATTERN", "B"].iter().map(|s| s.to_string()).collect(),
        vec!["ANOMALY", "PATTERN", "X"].iter().map(|s| s.to_string()).collect(),
        vec!["ANOMALY", "PATTERN", "Y"].iter().map(|s| s.to_string()).collect(),
    ];
    ...
    assert_eq!(results.len(), sequences.len(), "All sequences should be processed");
    
    // Verify mathematical properties for all results
    for (i, anomaly_set) in results.iter().enumerate() {
        for anomaly in anomaly_set {
            assert!(anomaly.likelihood >= 0.0 && anomaly.likelihood <= 1.0,
                   "Invalid likelihood in batch result {}: {}", i, anomaly.likelihood);
            assert!(anomaly.anomaly_strength >= 0.0 && anomaly.anomaly_strength <= 1.0,
                   "Invalid anomaly strength in batch result {}: {}", i, anomaly.anomaly_strength);
            assert!(anomaly.information_score >= 0.0,
                   "Invalid information score in batch result {}: {}", i, anomaly.information_score);
        }
    }
}
...
fn test_performance_monitoring_workflow() {
    ...
    let training_data = vec!["A", "B", "C", "D"].repeat(100)
        .iter().map(|s| s.to_string()).collect::<Vec<_>>();
    detector.train(&training_data).expect("Failed to train");
    
    // Check training metrics
    let metrics = detector.performance_metrics();
    assert!(metrics.training_time_ms >= 0, "Training time should be recorded");
    assert!(metrics.context_count > 0, "Context count should be positive");
    assert!(metrics.estimated_memory_bytes > 0, "Memory estimate should be positive");
    
    // Test detection with monitoring
    let test_data = vec!["X".to_string(), "Y".to_string(), "Z".to_string()];
    let _anomalies = detector.detect_anomalies_with_monitoring(&test_data, 0.1)
        .expect("Failed to detect with monitoring");
    
    // Check detection metrics
    let updated_metrics = detector.performance_metrics();
    assert!(updated_metrics.detection_time_ms > 0, "Detection time should be recorded");
}
...
fn test_optimization_workflow() {
    ...
    assert!(optimized_contexts <= initial_contexts,
           ...
           initial_contexts, optimized_contexts);
    
    // Detection should still work after optimization
    let test_sequence = vec!["STATE_0".to_string(), "NEXT_1".to_string(), "FINAL_2".to_string()];
...
fn test_threshold_sensitivity_workflow() {
    ...
    let normal_pattern = vec!["NORMAL", "FLOW"].repeat(100)
        .iter().map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    for &threshold in &thresholds {
        let anomalies = detector.detect_anomalies(&anomalous_pattern, threshold)
            .expect("Failed to detect anomalies");
