...
fn domain_3_information_theory() {
    ...
    println!();
    println!("Domain 3 Result: {}/{} tests passed", passed_tests, total_tests);
...
fn test_shannon_entropy_comprehensive() -> DomainTestResult {
    ...
    let deterministic_sequence = vec!["A"; 100].iter().map(|s| s.to_string()).collect::<Vec<_>>();
    detector1.train(&deterministic_sequence).expect("Failed to train");
    
    // Get a context node to test entropy calculation
    if let Some(context_node) = detector1.model().context_tree().get_context_node(&vec!["A".to_string()]) {
        ...
        if entropy_deterministic > 0.1 {
            violations += 1;
            details.push(format!("Deterministic entropy too high: {:.6} > 0.1", entropy_deterministic));
        }
        details.push(format!("Deterministic entropy: {:.6}", entropy_deterministic));
    }
    ...
    let uniform_sequence = vec!["A", "X", "A", "Y", "A", "Z", "A", "W"].repeat(50).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    detector2.train(&uniform_sequence).expect("Failed to train");
    
    if let Some(context_node) = detector2.model().context_tree().get_context_node(&vec!["A".to_string()]) {
        ...
        if entropy_uniform < theoretical_max * 0.8 { // Allow some tolerance for smoothing
            violations += 1;
            details.push(format!("Uniform entropy too low: {:.6} < {:.6}", entropy_uniform, theoretical_max * 0.8));
        }
        details.push(format!("Uniform entropy: {:.6}, theoretical max: {:.6}", entropy_uniform, theoretical_max));
    }
    ...
    let skewed_sequence = vec!["A", "X", "A", "X", "A", "X", "A", "Y"].repeat(50).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    detector3.train(&skewed_sequence).expect("Failed to train");
    
    if let Some(context_node) = detector3.model().context_tree().get_context_node(&vec!["A".to_string()]) {
    ...
    for (i, sequence) in test_sequences.iter().enumerate() {
        ...
        for state in &global_vocab {
            if let Some(context_node) = detector.model().context_tree().get_context_node(&vec![state.to_string()]) {
                let entropy = context_node.compute_entropy(detector.model().config());
                if entropy < 0.0 {
                    violations += 1;
                    details.push(format!("Negative entropy in sequence {}, context {}: {:.6}", i, state, entropy));
                }
            }
        }
    }
...
fn test_information_content_properties_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "A", "A", "A", "B", "B", "C"].repeat(100)
    ...
    for state in &states {
        let prob = detector.model().get_best_context_probability(&context, state);
        let info_content = if prob > 0.0 { -prob.log2() } else { f64::INFINITY };
        
        probabilities.push(prob);
        info_contents.push(info_content);
        
        println!("      P({} | A) = {:.6}, I({}) = {:.6}", state, prob, state, info_content);
        
        // Test non-negativity
        if info_content < 0.0 {
            violations += 1;
            details.push(format!("Negative information content for {}: {:.6}", state, info_content));
        }
    }
    ...
    let min_prob_idx = probabilities.iter()
        .enumerate()
    ...
    if max_prob_idx != min_prob_idx {
        ...
        details.push(format!("Info content ordering: rare={:.6}, common={:.6}", low_prob_info, high_prob_info));
    }
    ...
    if let Some(context_node) = detector.model().context_tree().get_context_node(&context) {
        ...
        let expected_info: f64 = all_probs.iter()
            .map(|(_, prob)| {
                if *prob > 0.0 {
                    prob * (-prob.log2())
                } else {
        ...
        println!("      Expected information (consistent): {:.6}", expected_info);
        println!("      Error: {:.6}", entropy_error);
        
        if entropy_error > 0.001 { // Tighter tolerance since we're using same source
            violations += 1;
            details.push(format!("Entropy-information mismatch: error = {:.6}", entropy_error));
        }
        
        details.push(format!("Entropy vs expected info: {:.6} vs {:.6}, error: {:.6}", 
...
fn test_kl_divergence_comprehensive() -> DomainTestResult {
    ...
    let mut detector1 = AnomalyDetector::new(1).expect("Failed to create detector");
    let uniform_sequence = vec!["A", "B", "C", "D"].repeat(100).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    if let Some(context_node) = detector1.model().context_tree().get_context_node(&vec!["A".to_string()]) {
        ...
        if kl_div_uniform > 0.1 {
            violations += 1;
            details.push(format!("KL divergence from uniform too high for uniform data: {:.6}", kl_div_uniform));
        }
    ...
    let mut detector2 = AnomalyDetector::new(1).expect("Failed to create detector");
    let skewed_sequence = vec!["A", "A", "A", "A", "A", "B"].repeat(100).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    ...
    for (i, sequence) in test_sequences.iter().enumerate() {
        ...
        for state in &global_vocab {
            if let Some(context_node) = detector.model().context_tree().get_context_node(&vec![state.to_string()]) {
                ...
                if kl_div < 0.0 {
                    violations += 1;
                    details.push(format!("Negative KL divergence in sequence {}, context {}: {:.6}", i, state, kl_div));
                }
            }
        }
    }
    ...
    for (sequence, description) in &distributions {
        ...
        if let Some(context_node) = detector.model().context_tree().get_context_node(&vec!["A".to_string()]) {
            let kl_div = context_node.compute_kl_divergence(detector.model().config());
            kl_divs.push(kl_div);
    ...
    if kl_divs.len() >= 2 {
        ...
        if ordering_violations > 0 {
            details.push(format!("KL divergence ordering violations: {}", ordering_violations));
        }
    }
...
fn test_cross_entropy_relationships_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "X", "A", "Y", "A", "Z"].repeat(100).iter().map(|s| s.to_string()).collect::<Vec<_>>();
    detector.train(&training_sequence).expect("Failed to train");
    ...
    if let Some(context_node) = detector.model().context_tree().get_context_node(&vec!["A".to_string()]) {
        ...
        if cross_entropy_error > 0.01 {
            violations += 1;
            details.push(format!("Cross-entropy relationship violation: error = {:.6}", cross_entropy_error));
        }
        
        details.push(format!("Cross-entropy relationship: H(P,Q)={:.6}, H(P)+D_KL={:.6}, error={:.6}", 
...
fn test_information_theory_inequalities() -> DomainTestResult {
    ...
    for (i, sequence) in test_sequences.iter().enumerate() {
        ...
        println!("    Testing sequence {} (alphabet size: {})", i, alphabet_size);
        ...
        for state in &global_vocab {
            if let Some(context_node) = detector.model().context_tree().get_context_node(&vec![state.to_string()]) {
                ...
                // Test H(X) ≤ log₂|X|
                if entropy > max_entropy + 0.01 { // Small tolerance for numerical errors
                    violations += 1;
                    details.push(format!("Entropy bound violation in seq {}, context {}: {:.6} > {:.6}", 
                                       i, state, entropy, max_entropy));
                }
                
                // Test D_KL(P||Q) ≥ 0
                if kl_div < -0.001 { // Small tolerance for numerical errors
                    violations += 1;
                    details.push(format!("KL divergence negativity in seq {}, context {}: {:.6}", 
                                       i, state, kl_div));
                }
                ...
                for test_state in &global_vocab {
                    let prob = detector.model().get_best_context_probability(&vec![state.to_string()], test_state);
                    if prob > 0.0 {
                ...
                // Test H(P,Q) ≥ H(P)
                if cross_entropy < entropy - 0.001 { // Small tolerance
                    violations += 1;
                    details.push(format!("Cross-entropy bound violation in seq {}, context {}: {:.6} < {:.6}", 
    ...
    details.push(format!("Tested {} sequences with various alphabet sizes", test_sequences.len()));
    
    if violations == 0 {
        DomainTestResult::pass("All information theory inequalities satisfied".to_string())
            .with_details(details)
    } else {
        DomainTestResult::fail(format!("{} information theory inequality violations", violations))
            .with_details(details)
    }
}