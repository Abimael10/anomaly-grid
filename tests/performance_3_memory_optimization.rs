//! Performance Test 3: Memory Optimization
//!
//! This test validates the memory optimization characteristics of the anomaly detection
//! library, including memory usage patterns, garbage collection behavior, and memory
//! efficiency under various workloads and configurations.
...
fn test_memory_usage_scaling_with_data_size() {
    ...
    for i in 1..memory_usages.len() {
        ...
        println!("Size ratio: {:.2}, Memory ratio: {:.2}", size_ratio, memory_ratio);
        
        // Memory should not grow faster than quadratic with data size
        assert!(memory_ratio <= size_ratio * size_ratio * 1.5,
...
fn test_memory_efficiency_with_alphabet_size() {
    ...
    for &alphabet_size in &alphabet_sizes {
        ...
        println!("Alphabet size: {}, Memory: {} KB, Contexts: {}, Efficiency: {:.1}%", 
                alphabet_size, memory_usage / 1024, context_count,
...
fn test_memory_optimization_with_max_order() {
    ...
    for &max_order in &max_orders {
        ...
        println!("Max order: {}, Memory: {} KB, Contexts: {}", 
                max_order, memory_usage / 1024, context_count);
...
fn test_memory_stability_during_operations() {
    ...
    println!("Initial state - Memory: {} KB, Contexts: {}", 
            initial_memory / 1024, initial_contexts);
    ...
    println!("Final state - Memory: {} KB, Contexts: {}", 
            final_memory / 1024, final_contexts);
    
    // Final validation
    let total_memory_change = (final_memory as f64 / initial_memory as f64 - 1.0).abs();
    assert!(total_memory_change < 0.02,
           "Total memory change {} exceeds 2% after {} operations",
           total_memory_change, operations);
    
    assert_eq!(final_contexts, initial_contexts,
...
fn test_memory_optimization_with_configuration() {
    ...
    let configurations = vec![
        ("Low memory", AnomalyGridConfig::default().with_max_order(2).unwrap().with_smoothing_alpha(1.0).unwrap()),
        ("Balanced", AnomalyGridConfig::default().with_max_order(3).unwrap().with_smoothing_alpha(0.5).unwrap()),
        ("High accuracy", AnomalyGridConfig::default().with_max_order(5).unwrap().with_smoothing_alpha(0.1).unwrap()),
    ];
    ...
    for (config_name, config) in configurations {
        ...
        println!("Configuration '{}': Memory: {} KB, Contexts: {}", 
                config_name, memory_usage / 1024, context_count);
        
        // Validate configuration-specific expectations
        match config_name {
            "Low memory" => {
                assert!(memory_usage < 200 * 1024, // 200KB limit for low memory
                       "Low memory config uses {} bytes, exceeds 200KB limit", memory_usage);
            },
            "Balanced" => {
                assert!(memory_usage < 500 * 1024, // 500KB limit for balanced
                       "Balanced config uses {} bytes, exceeds 500KB limit", memory_usage);
            },
            "High accuracy" => {
                assert!(memory_usage < 2 * 1024 * 1024, // 2MB limit for high accuracy
                       "High accuracy config uses {} bytes, exceeds 2MB limit", memory_usage);
            },
            _ => {}
        }
        ...
        if context_count > 0 {
            let memory_per_context = memory_usage / context_count;
            assert!(memory_per_context < 8192, // 8KB per context max
                   "Config '{}' memory per context {} bytes exceeds 8KB",
                   config_name, memory_per_context);
        }
    }
...
fn test_memory_fragmentation_resistance() {
    ...
    for i in 0..num_detectors {
        ...
        assert!(memory_usage < 200 * 1024, // 200KB per detector max
               "Detector {} uses {} bytes, exceeds 200KB limit", i + 1, memory_usage);
        
        detectors.push(detector);
    }
    
    println!("Total memory for {} detectors: {} KB", num_detectors, total_memory / 1024);
    
    // Total memory should be reasonable
    assert!(total_memory < 2 * 1024 * 1024, // 2MB total limit
           "Total memory {} bytes exceeds 2MB limit for {} detectors",
           total_memory, num_detectors);
...
fn test_memory_efficiency_metrics() {
    ...
    for (max_order, alphabet_size, training_size) in test_cases {
        ...
        let memory_per_context = if context_count > 0 { memory_usage / context_count } else { 0 };
        let memory_per_element = memory_usage as f64 / training_size as f64;
        
        println!("Order: {}, Alphabet: {}, Training: {}", max_order, alphabet_size, training_size);
        println!("  Memory: {} KB, Contexts: {}/{} ({:.1}%)", 
                memory_usage / 1024, context_count, theoretical_max, context_efficiency * 100.0);
        println!("  Memory/context: {} bytes, Memory/element: {:.1} bytes", 
                memory_per_context, memory_per_element);
        
        // Validate efficiency metrics
        assert!(context_efficiency < 1.0,
               "Context efficiency {} too high (potential waste) for order {} alphabet {}",
               context_efficiency, max_order, alphabet_size);
        
        assert!(memory_per_context < 4096,
               "Memory per context {} bytes exceeds 4KB for order {} alphabet {}",
               memory_per_context, max_order, alphabet_size);
        
        assert!(memory_per_element < 2048.0,
               "Memory per element {} bytes exceeds 2KB for order {} alphabet {}",
               memory_per_element, max_order, alphabet_size);
    }
...
fn calculate_memory_efficiency(alphabet_size: usize, max_order: usize, actual_contexts: usize) -> f64 {
    let theoretical_max = calculate_theoretical_max_contexts(alphabet_size, max_order);
