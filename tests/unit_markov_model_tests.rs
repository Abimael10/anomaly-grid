//! Unit tests for the MarkovModel implementation.
//! These tests verify the core functionality of the Markov model.
...
fn test_basic_training() {
    ...
    assert!(
        model.context_tree().context_count() > 0,
        "Model should have contexts"
    );
    assert!(
        !model.state_mapping().is_empty(),
        "Model should have state mapping"
    );
}
...
fn test_empty_context_handling() {
    ...
    assert!(
        prob > 0.0 && prob <= 1.0,
        "Empty context probability should be in (0,1]: {:.6}",
        prob
    );
}
...
fn test_model_configuration_access() {
    ...
    let model = MarkovModel::with_config(config.clone()).expect("Failed to create model");
...
fn test_numerical_stability_extreme_cases() {
    ...
    for (i, sequence) in extreme_cases.iter().enumerate() {
        ...
        assert!(
            train_result.is_ok(),
            "Training should succeed for case {}",
            i
        );
        ...
        assert!(
            likelihood >= 0.0 && likelihood <= 1.0,
            "Likelihood should be in bounds for case {}: {:.15}",
            i,
            likelihood
        );
    }
