//! Performance Test 4: Batch Processing
//!
//! This test validates the batch processing performance characteristics of the anomaly
//! detection library, including throughput optimization, parallel processing simulation,
//! and scalability under high-volume workloads.
...
fn test_batch_processing_throughput() {
    ...
    for &batch_size in &batch_sizes {
        ...
        println!("Batch size: {}, Throughput: {:.0} seq/sec, Avg latency: {:?}", 
        ...
        assert!(avg_latency.as_millis() < 10,
               "Average latency {} ms exceeds 10 ms threshold for batch size {}",
               avg_latency.as_millis(), batch_size);
...
fn test_batch_processing_scalability() {
    ...
    for i in 1..processing_times.len() {
        ...
        let time_ratio = processing_times[i].as_nanos() as f64 / processing_times[i-1].as_nanos() as f64;
        
        println!("Size ratio: {:.2}, Time ratio: {:.2}", size_ratio, time_ratio);
...
fn test_batch_processing_memory_efficiency() {
    ...
    for &batch_size in &batch_sizes {
        ...
        println!("After batch size {}: Memory usage: {} KB", 
                batch_size, current_memory / 1024);
...
fn test_batch_processing_with_different_sequence_lengths() {
    ...
    for &seq_length in &sequence_lengths {
        ...
        println!("Sequence length: {}, Throughput: {:.0} seq/sec, Time/element: {:.0} ns", 
        ...
        assert!(avg_time_per_element < 50000.0, // 50μs per element max
               "Time per element {} ns exceeds 50μs for length {}",
               avg_time_per_element, seq_length);
    }
...
fn test_batch_processing_concurrent_simulation() {
    ...
    println!("Concurrent simulation: {} workers, {} seq/worker", num_workers, sequences_per_worker);
    println!("Total sequences: {}, Total time: {:?}", total_sequences, total_time);
    println!("Throughput: {:.0} seq/sec, Avg latency: {:?}", throughput, avg_latency);
    ...
    assert!(avg_latency.as_micros() < 2000,
...
fn test_batch_processing_with_mixed_workloads() {
    ...
    for (workload_name, seq_length, batch_size) in workload_types {
        ...
        println!("Workload '{}': {} seq/sec, {} elements/sec", 
        ...
        assert!(throughput >= 50.0,
               ...
               workload_name, throughput);
        
        assert!(elements_per_sec >= 1000.0,
               "Workload '{}' element throughput {} elements/sec below 1000 threshold",
               workload_name, elements_per_sec);
    }
...
fn test_batch_processing_resource_utilization() {
    ...
    for batch_num in 0..sustained_batches {
        ...
        if batch_num % 5 == 4 {
            let current_metrics = detector.performance_metrics();
            println!("Batch {}: Throughput: {:.0} seq/sec, Memory: {} KB", 
                    batch_num + 1, batch_throughput, current_metrics.estimated_memory_bytes / 1024);
            
            // Resource utilization should remain stable
            let memory_ratio = current_metrics.estimated_memory_bytes as f64 / initial_metrics.estimated_memory_bytes as f64;
            assert!(memory_ratio < 1.1,
                   "Memory utilization grew {} times after batch {}",
                   memory_ratio, batch_num + 1);
        }
        
        // Each batch should maintain good performance
        assert!(batch_throughput >= 800.0,
               "Batch {} throughput {} seq/sec below 800 threshold",
               batch_num + 1, batch_throughput);
    }
    ...
    assert!(overall_throughput >= 1000.0,
           "Sustained throughput {} seq/sec below 1000 threshold",
           overall_throughput);
    
    let final_metrics = detector.performance_metrics();
    let final_memory_ratio = final_metrics.estimated_memory_bytes as f64 / initial_metrics.estimated_memory_bytes as f64;
    assert!(final_memory_ratio < 1.05,
