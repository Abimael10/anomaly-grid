...
//! capabilities, ensuring our implementation correctly handles various sequence
...
fn test_context_window_semantics_comprehensive() -> DomainTestResult {
    ...
    for &max_order in &context_lengths {
        ...
        let training_sequence = vec!["A", "B", "C", "A", "B", "D", "A", "B", "C", "A", "B", "D"]
            .repeat(50)
            .iter()
            .map(|s| s.to_string())
            .collect::<Vec<_>>();
        ...
        println!(
            "      Context length {}: max anomaly score = {:.6}",
            max_order, max_score
        );
    }
    ...
    let specificity_trend = pattern_specificity
        .windows(2)
        .map(|w| w[1] - w[0])
        .collect::<Vec<_>>();
    ...
    let training_sequence = vec!["START", "A", "B", "C", "END"]
        .repeat(20)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let boundary_errors = boundary_scores
        .iter()
        .filter(|&&score| score.is_nan() || score.is_infinite())
        .count();
    ...
    let sparse_training = vec!["A", "B", "C", "D", "E", "F", "G", "H"]
        .repeat(10)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
...
fn test_temporal_dependency_modeling_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["CAUSE", "EFFECT1", "EFFECT2", "RESET"]
        .repeat(100)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let strong_anomalies = strong_detector
        .detect_anomalies(&violation_sequence, 0.0)
        .unwrap_or_default();
    let weak_anomalies = weak_detector
        .detect_anomalies(&violation_sequence, 0.0)
        .unwrap_or_default();

    let strong_score = strong_anomalies
        .iter()
        .map(|a| a.anomaly_strength)
        .fold(0.0f64, f64::max);
    let weak_score = weak_anomalies
        .iter()
        .map(|a| a.anomaly_strength)
        .fold(0.0f64, f64::max);
    ...
    if strong_score < weak_score * 0.5 {
...
fn test_pattern_recognition_accuracy_comprehensive() -> DomainTestResult {
    ...
    let pattern_variations = vec![
        ...
        (
            vec!["PATTERN2", "X", "WRONG", "Z"],
            "Pattern 2 with variation",
        ),
    ...
    let variation_min = variation_scores
        .iter()
    ...
    if variation_min <= known_max {
        violations += 1;
        details.push(format!(
            "Pattern variations not detected: variation_min ({:.6}) <= known_max ({:.6})",
    ...
    if unknown_min <= known_max {
        violations += 1;
        details.push(format!(
            "Unknown patterns not detected: unknown_min ({:.6}) <= known_max ({:.6})",
...
fn test_sequence_length_handling_comprehensive() -> DomainTestResult {
    ...
    let short_sequences = vec![vec!["A"], vec!["A", "B"], vec!["B", "C"]];
    ...
    for sequence in &length_tests {
        ...
        println!(
            "      Length {}: anomaly score at position 2 = {:.6}",
            sequence.len(),
            anomaly_score
        );
    }
    ...
    if performance_times.len() >= 2 {
        ...
        if scaling_factor > 10.0 {
            // More than 10x slowdown is concerning
            violations += 1;
...
fn test_alphabet_size_scalability() -> DomainTestResult {
    ...
    match small_result {
        Ok(_) => {
            ...
            let anomalies = small_detector
                .detect_anomalies(&test_seq, 0.0)
                .unwrap_or_default();
            let small_score = anomalies
                .iter()
                .map(|a| a.anomaly_strength)
                .fold(0.0f64, f64::max);
            println!(
                "      Small alphabet (2 states): max anomaly score = {:.6}",
                small_score
            );
        }
        Err(e) => {
    ...
    match medium_result {
        Ok(_) => {
            ...
            let anomalies = medium_detector
                .detect_anomalies(&test_seq, 0.0)
                .unwrap_or_default();
            let medium_score = anomalies
                .iter()
                .map(|a| a.anomaly_strength)
                .fold(0.0f64, f64::max);
            println!(
                "      Medium alphabet (10 states): max anomaly score = {:.6}",
                medium_score
            );
        }
        Err(e) => {
    ...
    match large_result {
        Ok(_) => {
            ...
            let anomalies = large_detector
                .detect_anomalies(&test_seq, 0.0)
                .unwrap_or_default();
            let large_score = anomalies
                .iter()
                .map(|a| a.anomaly_strength)
                .fold(0.0f64, f64::max);
            println!(
                "      Large alphabet (50 states): max anomaly score = {:.6}",
                large_score
            );
        }
        Err(e) => {
    ...
    for &size in &alphabet_sizes {
        ...
        let training_data: Vec<String> = alphabet
            .iter()
            .cycle()
            .take(alphabet.len() * 20)
            .cloned()
            .collect();
        ...
        match train_result {
            Ok(_) => {
                ...
                println!(
                    "      Alphabet size {}: train={}ms, detect={}ms",
                    size,
    ...
    let efficiency_tests = vec![
        ...
        (
            (0..8).map(|i| format!("{}", i)).collect::<Vec<String>>(),
            "Octal alphabet",
        ),
        (
            (0..16).map(|i| format!("{:X}", i)).collect::<Vec<String>>(),
            "Hex alphabet",
        ),
    ];
