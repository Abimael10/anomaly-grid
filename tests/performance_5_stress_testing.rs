...
//!
//! This test validates the stress testing characteristics of the anomaly detection
//! library under extreme conditions, including high-volume workloads, edge cases,
//! and sustained operation under maximum load conditions.
...
fn test_high_volume_stress_testing() {
    ...
    detector.train(&large_training_data).expect("Training should succeed");
    let train_time = train_start.elapsed();
    
    let metrics = detector.performance_metrics();
    println!("Training completed in {:?}", train_time);
    println!("Contexts learned: {}, Memory: {} KB", 
            metrics.context_count, metrics.estimated_memory_bytes / 1024);
    
    // Validate training performance under stress
    assert!(train_time.as_secs() < 30,
           "Training time {} sec exceeds 30 sec limit for large dataset",
           train_time.as_secs());
    
    assert!(metrics.estimated_memory_bytes < 100 * 1024 * 1024, // 100MB limit
    ...
    println!("Performing high volume detection: {} sequences...", stress_sequences);
    ...
    for i in 0..stress_sequences {
        ...
        if i % 1000 == 999 {
            ...
            println!("  Processed {} sequences, throughput: {:.0} seq/sec", 
                    i + 1, throughput);
        }
    }
    ...
    assert!(overall_throughput >= 200.0,
           "Stress test throughput {} seq/sec below 200 threshold",
           overall_throughput);
    
    assert!(total_detection_time.as_secs() < 60,
...
fn test_sustained_load_stress_testing() {
    ...
    println!("Running sustained load test for {} seconds at {} seq/sec...", 
    ...
    while test_start.elapsed() < test_duration {
        ...
        for i in 0..batch_size {
            let test_sequence = generate_stress_test_sequence(
                sequence_length, 15, sequences_processed + i
            );
            let anomalies = detector.detect_anomalies(&test_sequence, 0.1)
                .expect("Detection should succeed");
            total_anomalies += anomalies.len();
        ...
        if sequences_processed % (target_throughput * 5) == 0 && sequences_processed > 0 {
            ...
            println!("  {} sec: {} sequences, {:.0} seq/sec, {} KB memory", 
                    elapsed.as_secs(), sequences_processed, current_throughput,
                    current_metrics.estimated_memory_bytes / 1024);
            
            // Validate sustained performance
            assert!(current_throughput >= target_throughput as f64 * 0.8,
                   "Sustained throughput {} seq/sec below 80% of target {}",
                   current_throughput, target_throughput);
            
            // Validate memory stability
            let memory_growth = current_metrics.estimated_memory_bytes as f64 / initial_memory as f64;
            assert!(memory_growth < 1.2,
    ...
    let avg_batch_throughput = performance_samples.iter().sum::<f64>() / performance_samples.len() as f64;
    let min_batch_throughput = performance_samples.iter().fold(f64::INFINITY, |a, &b| a.min(b));
    ...
    println!("  Batch throughput - Avg: {:.0}, Min: {:.0}, Max: {:.0}", 
            avg_batch_throughput, min_batch_throughput, max_batch_throughput);
    println!("  Total anomalies: {}", total_anomalies);
    
    // Final validation
    assert!(final_throughput >= target_throughput as f64 * 0.9,
           "Final throughput {} seq/sec below 90% of target {}",
           final_throughput, target_throughput);
    
    assert!(min_batch_throughput >= target_throughput as f64 * 0.5,
    ...
    assert!(final_memory_growth < 1.1,
...
fn test_extreme_configuration_stress() {
    ...
    let extreme_configs = vec![
        ("High order", AnomalyGridConfig::default().with_max_order(8).unwrap()),
        ("Large alphabet", AnomalyGridConfig::default().with_max_order(3).unwrap()),
        ("Low smoothing", AnomalyGridConfig::default().with_max_order(4).unwrap().with_smoothing_alpha(0.01).unwrap()),
        ("High smoothing", AnomalyGridConfig::default().with_max_order(3).unwrap().with_smoothing_alpha(10.0).unwrap()),
    ];
    ...
    for (config_name, config) in extreme_configs {
        ...
        println!("  Contexts: {}, Memory: {} KB", 
                metrics.context_count, metrics.estimated_memory_bytes / 1024);
        
        // Validate extreme configuration performance
        assert!(train_time.as_secs() < 60,
               "Extreme config '{}' training time {} sec exceeds 60 sec",
               config_name, train_time.as_secs());
        
        assert!(metrics.estimated_memory_bytes < 500 * 1024 * 1024, // 500MB limit
               "Extreme config '{}' memory {} bytes exceeds 500MB",
               config_name, metrics.estimated_memory_bytes);
        ...
        println!("  Detection throughput: {:.0} seq/sec", detection_throughput);
        
        // Validate detection performance under extreme conditions
        assert!(detection_throughput >= 10.0,
               "Extreme config '{}' detection throughput {} seq/sec below 10",
               config_name, detection_throughput);
    }
...
fn test_memory_pressure_stress() {
    ...
    println!("Creating memory pressure with {} states and {} training elements...", 
    ...
    println!("  Successful detections: {}/{} ({:.1}%)", 
            successful_detections, stress_detection_sequences, success_rate * 100.0);
...
fn test_edge_case_stress_combinations() {
    ...
    let edge_case_combinations = vec![
        ...
        ("Single element sequences", vec![vec!["STATE_0".to_string()]]),
        ("Very long sequences", vec![vec!["STATE_0".to_string(); 1000]]),
        ("Highly repetitive", vec![vec!["REPEAT".to_string(); 100]]),
        ("Unknown elements", vec![vec!["UNKNOWN_1".to_string(), "UNKNOWN_2".to_string()]]),
        ("Mixed edge cases", vec![
            vec![],
    ...
    for (case_name, test_sequences) in edge_case_combinations {
        ...
        println!("  Results: {}/{} successful ({:.1}%), {:.0} seq/sec", 
                successful_detections, total_detections, success_rate * 100.0, throughput);
        
        // Validate edge case handling
        assert!(success_rate >= 0.9,
               "Edge case '{}' success rate {} below 90%",
               case_name, success_rate);
        
        assert!(throughput >= 1000.0,
               "Edge case '{}' throughput {} seq/sec below 1000",
               case_name, throughput);
    }
...
fn test_concurrent_stress_simulation() {
    ...
    println!("Simulating {} concurrent workers with {} sequences each...", 
    ...
    for worker in 0..simulated_workers {
        ...
        println!("  Worker {}: {:?}, {:.0} seq/sec", 
                worker + 1, worker_time, worker_throughput);
    }
    ...
    let avg_worker_time = worker_times.iter().sum::<std::time::Duration>() / worker_times.len() as u32;
    ...
    println!("  Successful: {} ({:.1}%)", successful_detections, success_rate * 100.0);
    println!("  Overall throughput: {:.0} seq/sec", overall_throughput);
    println!("  Worker times - Avg: {:?}, Min: {:?}, Max: {:?}", 
