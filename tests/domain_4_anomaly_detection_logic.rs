...
fn test_anomaly_definition_consistency_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "B", "C"]
        .repeat(100)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    for (i, sequence) in normal_sequences.iter().enumerate() {
        ...
        let anomalies = detector
            .detect_anomalies(&seq_strings, 0.0)
            .unwrap_or_default();
        ...
        println!(
            "      Normal sequence {}: max anomaly score = {:.6}",
            i, max_score
        );
    }
    ...
    for (i, sequence) in anomalous_sequences.iter().enumerate() {
        ...
        println!(
            "      Anomalous sequence {}: max anomaly score = {:.6}",
            i, max_score
        );
    }
    ...
    let anomalous_min = anomalous_scores
        .iter()
    ...
    if anomalous_min <= normal_max {
        violations += 1;
        details.push(format!(
            "Score ordering violation: anomalous_min ({:.6}) <= normal_max ({:.6})",
    ...
    for &threshold in &thresholds {
        ...
        println!(
            "      Threshold {:.1}: {} anomalies detected",
            threshold,
            anomalies.len()
        );
    }
    ...
    let monotonic_violations = detection_counts.windows(2).filter(|w| w[1] > w[0]).count();
    ...
    if violations == 0 {
        DomainTestResult::pass("Anomaly definition consistent".to_string()).with_details(details)
    } else {
...
fn test_likelihood_based_detection_comprehensive() -> DomainTestResult {
    ...
    let high_anomalies = detector
        .detect_anomalies(&high_likelihood_seq, 0.0)
        .unwrap_or_default();
    let low_anomalies = detector
        .detect_anomalies(&low_likelihood_seq, 0.0)
        .unwrap_or_default();

    let high_score = high_anomalies
        .iter()
        .map(|a| a.anomaly_strength)
        .fold(0.0f64, f64::max);
    let low_score = low_anomalies
        .iter()
        .map(|a| a.anomaly_strength)
        .fold(0.0f64, f64::max);
    ...
    println!(
        "      Score difference (low - high likelihood): {:.6}",
        score_difference
    );

    if score_difference < 0.001 {
        // Should be significant difference (adjusted threshold)
        violations += 1;
        details.push(format!(
            "Insufficient sensitivity: score difference = {:.6}",
            score_difference
        ));
    }
    ...
    if violations == 0 {
        ...
        DomainTestResult::fail(format!(
            "{} likelihood-based detection violations",
            violations
        ))
        .with_details(details)
    }
...
fn test_information_theoretic_scoring_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["COMMON", "COMMON", "COMMON", "RARE"]
        .repeat(100)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    for (sequence, description) in &test_cases {
        ...
        println!(
            "      {}: max information score = {:.6}",
            description, max_info
        );
    }
    ...
    for (sequence, description) in &test_cases {
        ...
        println!(
            "      {}: max anomaly strength = {:.6}",
            description, max_strength
        );
    }
    ...
    if negative_info_scores > 0 {
        violations += 1;
        details.push(format!(
            "Negative information scores: {}",
            negative_info_scores
        ));
    }

    // Information scores should be finite
    let infinite_info_scores = info_scores
        .iter()
    ...
    if infinite_info_scores > 0 {
        violations += 1;
        details.push(format!(
            "Infinite information scores: {}",
            infinite_info_scores
        ));
    }
    ...
    if violations == 0 {
        ...
        DomainTestResult::fail(format!(
            "{} information-theoretic scoring violations",
            violations
        ))
        .with_details(details)
    }
...
fn test_threshold_semantics_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["NORMAL", "NORMAL", "ANOMALY"]
        .repeat(100)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let all_anomalies = detector
        .detect_anomalies(&test_sequence, 0.0)
        .unwrap_or_default();
    ...
    for &threshold in &[0.1, 0.5, 0.9] {
        let expected_count = all_scores
            .iter()
            .filter(|&&score| score >= threshold)
            .count();
        let actual_anomalies = detector
            .detect_anomalies(&test_sequence, threshold)
            .unwrap_or_default();
        ...
        if expected_count != actual_count {
            violations += 1;
            details.push(format!(
                "Threshold precision violation at {:.1}: expected {}, got {}",
    ...
    let very_low_anomalies = detector
        .detect_anomalies(&test_sequence, 0.01)
        .unwrap_or_default();

    // Very high threshold (0.99) should catch almost nothing
    let very_high_anomalies = detector
        .detect_anomalies(&test_sequence, 0.99)
        .unwrap_or_default();

    println!(
        "      Very low threshold (0.01): {} detections",
        very_low_anomalies.len()
    );
    println!(
        "      Very high threshold (0.99): {} detections",
        very_high_anomalies.len()
    );
...
fn test_anomaly_strength_consistency() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "B", "A", "C", "A", "B", "A", "D"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    for (i, sequence) in test_sequences.iter().enumerate() {
        ...
        for anomaly in &anomalies {
            ...
            if anomaly.anomaly_strength < 0.0 || anomaly.anomaly_strength > 1.0 {
                violations += 1;
                details.push(format!(
                    "Strength out of range in sequence {}: {:.6}",
    ...
    let normal_anomalies = detector
        .detect_anomalies(&normal_seq, 0.0)
        .unwrap_or_default();
    let anomalous_anomalies = detector
        .detect_anomalies(&anomalous_seq, 0.0)
        .unwrap_or_default();

    let normal_max = normal_anomalies
        .iter()
        .map(|a| a.anomaly_strength)
        .fold(0.0f64, f64::max);
    let anomalous_max = anomalous_anomalies
        .iter()
        .map(|a| a.anomaly_strength)
        .fold(0.0f64, f64::max);
    ...
    println!(
        "      Anomalous sequence max strength: {:.6}",
        anomalous_max
    );
    ...
    if anomalous_max <= normal_max {
        violations += 1;
        details.push(format!(
            "Strength ordering violation: anomalous ({:.6}) <= normal ({:.6})",
    ...
    let test_anomalies = detector
        .detect_anomalies(&test_seq, 0.0)
        .unwrap_or_default();

    for (i, anomaly) in test_anomalies.iter().enumerate() {
        println!(
            "      Anomaly {}: strength={:.6}, likelihood={:.6}, info={:.6}",
        ...
        if anomaly.likelihood > 0.9 && anomaly.anomaly_strength > 0.5 {
            violations += 1;
            details.push(format!(
                "High likelihood ({:.6}) with high strength ({:.6})",
    ...
    if !all_strengths.is_empty() {
        ...
        println!(
            "      Strength distribution: min={:.6}, max={:.6}, mean={:.6}",
        ...
        details.push(format!(
            "Strength stats: min={:.6}, max={:.6}, mean={:.6}",
            min_strength, max_strength, mean_strength
        ));
    }
