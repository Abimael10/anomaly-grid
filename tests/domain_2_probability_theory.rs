...
fn test_kolmogorov_axioms_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "B", "C", "D", "E"]
        .repeat(100)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    for context in &contexts {
        ...
        for state in &global_vocab {
            let prob = detector
                .model()
                .get_best_context_probability(&context_strings, state);
            if prob < 0.0 {
                axiom_violations += 1;
                details.push(format!(
                    "Axiom 1 violation: P({} | {:?}) = {:.6} < 0",
                    state, context, prob
                ));
            }
        }
    ...
    for context in &contexts {
        ...
        if normalization_error > 1e-6 {
            axiom_violations += 1;
            details.push(format!(
                "Axiom 2 violation: P(Ω | {:?}) = {:.6} ≠ 1",
                context, total_prob
            ));
        }

        println!(
            "      Context {:?}: total probability = {:.6}",
            context, total_prob
        );
    }
    ...
    let prob_b = detector
        .model()
        .get_best_context_probability(&context_a, "B");
    let prob_c = detector
        .model()
        .get_best_context_probability(&context_a, "C");
    ...
    details.push(format!(
        "P(B|A) = {:.6}, P(C|A) = {:.6}, P(B∪C|A) = {:.6}",
        prob_b, prob_c, prob_union_bc
    ));

    if axiom_violations == 0 {
        DomainTestResult::pass("All Kolmogorov axioms satisfied".to_string()).with_details(details)
    } else {
...
fn test_conditional_probability_rules_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["A", "B", "A", "C", "B", "D", "B", "E"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    let prob_a_given_b = detector
        .model()
        .get_best_context_probability(&context_b, "A");
    let prob_c_given_b = detector
        .model()
        .get_best_context_probability(&context_b, "C");
    let prob_d_given_b = detector
        .model()
        .get_best_context_probability(&context_b, "D");
    let prob_e_given_b = detector
        .model()
        .get_best_context_probability(&context_b, "E");
    ...
    for state in &global_vocab {
        let prob = detector
            .model()
            .get_best_context_probability(&context_b, state);
    ...
    if conditional_normalization_error > 1e-6 {
        rule_violations += 1;
        details.push(format!(
            "Conditional normalization violation: sum = {:.6}",
            total_conditional
        ));
    }
    ...
    let prob_a_given_b_check = detector
        .model()
        .get_best_context_probability(&context_b, "A");
    ...
    if rule_violations == 0 {
        ...
        DomainTestResult::fail(format!(
            "{} conditional probability rule violations",
            rule_violations
        ))
        .with_details(details)
    }
...
fn test_bayes_theorem_application_comprehensive() -> DomainTestResult {
    ...
    if bayes_error > bayes_tolerance {
        bayes_violations += 1;
        details.push(format!(
            "Bayes theorem violation: error = {:.6} > {:.6}",
            bayes_error, bayes_tolerance
        ));
    }
    ...
    details.push(format!(
        "Bayes calculated: {:.6}, Direct: {:.6}, Error: {:.6}",
    ...
    if reverse_error > bayes_tolerance {
        bayes_violations += 1;
        details.push(format!(
            "Reverse Bayes violation: error = {:.6}",
            reverse_error
        ));
    }

    if bayes_violations == 0 {
        DomainTestResult::pass("Bayes' theorem correctly applied".to_string()).with_details(details)
    } else {
...
fn test_law_of_total_probability_comprehensive() -> DomainTestResult {
    ...
    let training_sequence = vec!["X", "A", "Y", "A", "Z", "A", "X", "B", "Y", "B", "Z", "C"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    ...
    for partition_state in &partition_states {
        ...
        let prob_a_given_bi = detector
            .model()
            .get_best_context_probability(&context, target_state);
        conditional_probs.push(prob_a_given_bi);

        // Estimate marginal probability P(Bᵢ)
        let count_bi = training_sequence
            .iter()
            .filter(|&s| s == partition_state)
            .count();
        ...
        println!(
            "      P(A|{}) = {:.6}, P({}) = {:.6}",
            partition_state, prob_a_given_bi, partition_state, marginal_prob_bi
        );
    }

    // Calculate total probability using the law
    let total_prob_calculated: f64 = conditional_probs
        .iter()
    ...
    println!(
        "      Total probability calculated: {:.6}",
        total_prob_calculated
    );
    ...
    if total_prob_error > tolerance {
        ...
        details.push(format!(
            "Total probability law violation: error = {:.6}",
            total_prob_error
        ));
    }

    details.push(format!(
        "Calculated: {:.6}, Direct: {:.6}, Error: {:.6}",
    ...
    if total_prob_violations == 0 {
        ...
        DomainTestResult::fail(format!(
            "{} total probability violations",
            total_prob_violations
        ))
        .with_details(details)
    }
}
...
fn test_independence_and_dependence() -> DomainTestResult {
    ...
    let independent_sequence = vec!["A", "X", "A", "Y", "B", "X", "B", "Y", "C", "X", "C", "Y"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    detector1
        .train(&independent_sequence)
    ...
    let dependent_sequence = vec!["A", "X", "A", "X", "B", "Y", "B", "Y"]
        .repeat(50)
        .iter()
        .map(|s| s.to_string())
        .collect::<Vec<_>>();
    detector2
        .train(&dependent_sequence)
        .expect("Failed to train");
    ...
    let prob_x_given_a1 = detector1
        .model()
        .get_best_context_probability(&vec!["A".to_string()], "X");
    let prob_x_given_b1 = detector1
        .model()
        .get_best_context_probability(&vec!["B".to_string()], "X");
    let prob_x_given_c1 = detector1
        .model()
        .get_best_context_probability(&vec!["C".to_string()], "X");
    ...
    details.push(format!(
        "Independence variance: {:.6}",
        independence_variance
    ));

    // Test dependence in detector2
    println!("    Testing dependence scenario");
    let prob_x_given_a2 = detector2
        .model()
        .get_best_context_probability(&vec!["A".to_string()], "X");
    let prob_x_given_b2 = detector2
        .model()
        .get_best_context_probability(&vec!["B".to_string()], "X");
    let prob_y_given_a2 = detector2
        .model()
        .get_best_context_probability(&vec!["A".to_string()], "Y");
    let prob_y_given_b2 = detector2
        .model()
        .get_best_context_probability(&vec!["B".to_string()], "Y");
    ...
    details.push(format!(
        "Dependence difference P(X|A) - P(X|B): {:.6}",
        dependence_difference
    ));
    ...
    if independence_violations == 0 {
        ...
        DomainTestResult::fail(format!(
            "{} independence/dependence violations",
