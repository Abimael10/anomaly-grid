...
//! implementation, including probability conservation and entropy calculations.
...
fn test_context_tree_building() {
    ...
    assert!(tree.context_count() > 0, "Context tree should have contexts");
}
...
fn test_kl_divergence_properties() {
    ...
    for (context, node) in &tree.contexts() {
        ...
        assert!(kl_divergence.is_finite(),
...
fn test_transition_probabilities() {
    ...
    assert!(prob_ab.unwrap() > 0.0, "Transition probability should be positive");
    
    let prob_ac = tree.get_transition_probability(&["A".to_string()], "C");
    assert!(prob_ac.is_some(), "Transition A->C should exist");
    assert!(prob_ac.unwrap() > 0.0, "Transition probability should be positive");
    
    // Test that A->B has higher probability than A->C (B appears twice, C once)
    assert!(prob_ab.unwrap() > prob_ac.unwrap(), "A->B should be more likely than A->C");
}
...
fn test_memory_estimation() {
    ...
    let sequence = vec!["A", "B", "C", "D"].repeat(10)
        .iter().map(|s| s.to_string()).collect::<Vec<_>>();
...
fn test_context_statistics() {
    ...
    assert!(stats.avg_entropy >= 0.0, "Average entropy should be non-negative");
    assert!(stats.max_entropy >= stats.avg_entropy, "Max entropy should be >= average");
}
...
fn test_deterministic_vs_uniform_entropy() {
    ...
    let det_sequence = vec!["A", "B"].repeat(50)
        .iter().map(|s| s.to_string()).collect::<Vec<_>>();
    det_tree.build_from_sequence(&det_sequence, &config)
    ...
    let uniform_sequence = vec!["A", "X", "A", "Y", "A", "Z", "A", "W"].repeat(25)
        .iter().map(|s| s.to_string()).collect::<Vec<_>>();
    uniform_tree.build_from_sequence(&uniform_sequence, &config)
...
fn test_single_element_sequence() {
    ...
    if result.is_ok() {
        assert!(tree.context_count() >= 0, "Context count should be non-negative");
    }
}